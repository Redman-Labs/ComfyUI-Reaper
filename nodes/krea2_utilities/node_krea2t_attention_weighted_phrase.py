...
from .shared_krea2_utilities._attention_weighting_layout import (
...
class ReaperKrea2TAttentionWeightedPhrase(io.ComfyNode):
    """Encode weighted Krea2 phrases and patch shared attention odds."""

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="ReaperKrea2TAttentionWeightedPhrase",
            display_name="Krea2T Text Encode - Weighted",
            category=CATEGORIES["krea2_utilities"],
            description=(
                "Encodes (phrase:weight) as ordinary Krea2 text, then applies "
                "the weight inside shared attention. Only image queries reading "
                "the phrase's text keys receive log(weight); conditioning rows "
                "and sequence length are unchanged."
            ),
            search_aliases=[
                "Krea2 Text Encode Attention-Weighted Phrases",
                "Krea2 weighted prompt",
                "Krea2 phrase weighting",
                "Reaper",
            ],
            inputs=[
                io.Model.Input(
                    ...
                    display_name="Model",
                    tooltip=(
                        "Krea2 MODEL to patch. Connect this output to the sampler."
                    ),
                ),
                io.Clip.Input(
                    "clip",
                    display_name="CLIP",
                    tooltip="Text encoder loaded with the Krea2 CLIP type.",
                ),
                io.String.Input(
                    "text",
                    display_name="Text",
                    ...
                    tooltip=(
                        "Use (phrase:weight). Weight 1 is unchanged, 0 suppresses "
                        "the phrase's attention odds, and values above 1 increase "
                        "them without copying or scaling conditioning rows."
                    ),
                ),
            ],
            outputs=[
                io.Model.Output(
                    ...
                    display_name="Model",
                    tooltip=(
                        "Krea2 MODEL whose shared attention uses the phrase weights."
                    ),
                ),
                io.Conditioning.Output(
                    ...
                    tooltip=(
                        "Normal, unscaled Krea2 conditioning for the clean prompt."
                    ),
                ),
                io.String.Output(
                    id="weight_report",
                    display_name="Weight Report",
                    tooltip=(
                        "JSON report of phrase rows, weights, and attention behavior."
                    ),
                ),
            ],
        )
    ...
    @classmethod
    def execute(cls, model, clip, text: str) -> io.NodeOutput:
        if clip is None:
        ...
        if prompt_ids != layout["full_token_ids"]:
            raise ValueError(
                "The connected CLIP tokenizer does not match the verified local "
                "Krea2 Qwen tokenizer. No phrase weighting was installed."
            )
        ...
        report = _report(layout, block_count, active)
        if not active:
            return io.NodeOutput(model, output_conditioning, report)
        ...
        transformer_options["optimized_attention_override"] = (
            _make_attention_override(
                layout["row_weights"],
                previous_override,
                block_count,
            )
        )
        return io.NodeOutput(patched_model, output_conditioning, report)


__all__ = ["ReaperKrea2TAttentionWeightedPhrase"]
