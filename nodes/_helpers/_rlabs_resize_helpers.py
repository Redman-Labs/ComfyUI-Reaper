"""Shared image-resize engine for Reaper nodes.

Single source of truth for resize math used by Load Image Reaper AND
Image Resize Reaper. Operates on PIL images + a plain state dict so it is
framework-agnostic. JS mirror lives in js/load_image/resize_panel.mjs
