"""Private loading engine for Load Images from Folder (Reaper).

A "folder version" of Load Image Reaper: point it at any folder on disk, pick
...
from ._rlabs_resize_helpers import _resize_frame
...
DEFAULT_STATE = {
    ...
    # path instead ("sub/cat"), which lets Save Image Reaper rebuild the same
...
def _parse_state(state_json: str) -> dict:
    ...
    try:
        ...
        print("[ReaperLoadImagesFolder] Malformed state JSON, using defaults")
        return dict(DEFAULT_STATE)
...
class _ReaperLoadImagesFolderEngine:
    DESCRIPTION = (
        ...
        "options as Load Image Reaper (max megapixels, longest side, scale by, "
    ...
    def load(self, LoadImagesFolderState: str = ""):
        ...
        if not _red_prescreen(folder):
            raise ValueError(_red_denied_message(str(folder)))
        ...
        if not _red_folder_allowed(folder):
            raise ValueError(_red_denied_message(folder))
        if not selected:
            raise ValueError(
        ...
        for rel in selected:
            ...
            if _red_rel_is_rooted(rel):
                print(f"[ReaperLoadImagesFolder] not a relative name, skipped: {rel}")
                continue
            path = os.path.realpath(os.path.join(folder, rel))
            try:
                if os.path.commonpath([path, real_folder]) != real_folder:
                    print(f"[ReaperLoadImagesFolder] outside folder, skipped: {rel}")
                    continue
            except ValueError:
                continue  # different drive on Windows
            if not os.path.isfile(path):
                print(f"[ReaperLoadImagesFolder] missing, skipped: {rel}")
                continue
            try:
                t, m, fw, fh = _load_one(path, state, dtype)
            except Exception as e:
                print(f"[ReaperLoadImagesFolder] failed to load {rel}: {e}")
                continue
            images.append(t)
            masks.append(m)
            widths.append(fw)
    ...
    def IS_CHANGED(cls, LoadImagesFolderState: str = ""):
        ...
        if not _red_prescreen(folder) or not _red_folder_allowed(folder):
            return hashlib.sha256(b"reaper:folder-not-approved").hexdigest()
        real_folder = os.path.realpath(folder)
        ...
        for rel in state.get("selected", []) or []:
            ...
            if _red_rel_is_rooted(rel):     # before realpath - see load()
