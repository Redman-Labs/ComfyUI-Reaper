...
(`_resolve_reaper_path`, `_load_server_image`'s realpath+startswith, and
...
----------------------
1. FIXED roots we own (input/reaper, output, temp). Use `is_path_under`.
...
Config file: <ComfyUI user dir>/reaper/allowed_folders.json
...
def _config_path() -> str:
    """<ComfyUI user dir>/reaper/allowed_folders.json.
    ...
    `ComfyUI-Reaper/user/reaper/` inside the repo, where nothing was
    ...
    try:
        ...
        base = None
    if not base:
        base = os.path.join(os.path.expanduser("~"), ".reaper")
    ...
    return os.path.join(base, "reaper", _CONFIG_NAME)
...
def denied_message(path: str) -> str:
    ...
    return (
        "[Reaper] That folder is not approved, so it was not used.\n"
        "  Folder: {path}\n".format(path=path)
        + steps
