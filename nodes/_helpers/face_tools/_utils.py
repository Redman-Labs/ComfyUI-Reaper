...
_FACE_LANDMARKER_MODEL = os.path.join(models_dir, "mediapipe", "face_landmarker.task")
...
def _face_landmarker_model_path():
    if os.path.isfile(_FACE_LANDMARKER_MODEL):
        return _FACE_LANDMARKER_MODEL
    os.makedirs(os.path.dirname(_FACE_LANDMARKER_MODEL), exist_ok=True)
    temporary = _FACE_LANDMARKER_MODEL + ".download"
    try:
        print("[Reaper] Downloading MediaPipe Face Landmarker model")
        urllib.request.urlretrieve(_FACE_LANDMARKER_URL, temporary)
        os.replace(temporary, _FACE_LANDMARKER_MODEL)
    finally:
...
def _face_landmarker():
    options = mp_vision.FaceLandmarkerOptions(
        base_options=mp_python.BaseOptions(model_asset_path=_face_landmarker_model_path()),
...
def get_face_mesh(crop: torch.Tensor):
    rgb = np.ascontiguousarray(
        crop.clamp(0, 1).mul(255).to(torch.uint8)[0].detach().cpu().numpy()
    )
    image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
    with _FACE_LANDMARKER_LOCK:
        result = _face_landmarker().detect(image)
    _, h, w, _ = crop.shape
    if result.face_landmarks:
        all_pts = np.asarray(
            [[(w * landmark.x, h * landmark.y) for landmark in landmarks]
...
def mask_BiSeNet(crop,
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    with torch.no_grad():
        if not hasattr(Models, "_bisenet"):
            bisenet = BiSeNet(n_classes=19)
            model_path = os.path.join(models_dir, 'bisenet', '79999_iter.pth')
            state = torch.load(model_path, map_location=device, weights_only=True)
            bisenet.load_state_dict(state)
            Models._bisenet = bisenet.to(device).eval()
        bisenet = Models._bisenet
        crop_t = crop.permute(0,3,1,2).to(device=device, dtype=torch.float32)
    ...
    if not keep:
        return torch.zeros(crop.shape[:3], dtype=torch.float32)
    face_part_ids = torch.tensor(keep, device=device)
