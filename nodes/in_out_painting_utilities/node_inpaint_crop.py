...
from .._helpers._inpaint_helpers import (
    apply_inpaint_crop, merge_params, resolve_inpaint_mask, REAPER_CROP_INFO, DEFAULTS,
)
...
class ReaperInpaintCrop(io.ComfyNode):
    DESCRIPTION = (
        "Inpaint Crop Reaper - the easy way to set up an inpaint. Open the "
        ...
        "edit models), then send the crop_info wire into Inpaint Stitch Reaper "
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="ReaperInpaintCrop", display_name=node_title("Inpaint Crop"),
            category=CATEGORIES["in_out_painting_utilities"], description=cls.DESCRIPTION,
            is_output_node=True, accept_all_inputs=True,
            inputs=[
                io.Combo.Input("size_mode", options=list(_SIZE_MODE), default="keep shape (long side)"),
                io.Int.Input("target", default=1024, min=64, max=8192, step=8),
                io.Combo.Input("multiple", options=[8, 16, 32, 64], default=8),
                io.Int.Input("context_px", default=24, min=0, max=1024),
                io.Int.Input("mask_grow", default=4, min=0, max=256),
                io.Int.Input("mask_blur", default=4, min=0, max=256),
                io.Int.Input("softness", default=16, min=0, max=150),
                io.Combo.Input("blend_mode", options=list(_BLEND_MODE), default="mask"),
                io.Boolean.Input("invert_mask", default=False),
                io.Image.Input("image", optional=True), io.Mask.Input("mask", optional=True),
            ],
            outputs=[
                io.Image.Output(id="image", display_name="image"),
                io.Mask.Output(id="mask", display_name="mask"),
                CropInfo.Output(id="crop_info", display_name="crop_info"),
    ...
    RETURN_TYPES = ("IMAGE", "MASK", REAPER_CROP_INFO, "INT", "INT")
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "Crop info for Inpaint Stitch Reaper - carries the original image and "
    ...
    def IS_CHANGED(cls, **kwargs):
        ...
        try:
            ...
            if mp:
                full = cls._resolve_reaper_path(mp)
                if full:
                    parts.append(str(os.path.getmtime(full)))
            sp = meta.get("src_path", "")
            if sp:
                fs = cls._resolve_reaper_path(sp)
                if fs:
    ...
    def _save_source_temp(self, tensor):
        ...
        try:
            ...
            fname = f"reaper_inpaint_src_{uuid.uuid4().hex}.png"
            Image.fromarray(arr).save(os.path.join(temp_dir, fname), "PNG")
            return fname
        except Exception as e:
            print(f"[ReaperInpaintCrop] temp source save failed: {e}")
            return None

    def _load_disk_image(self, rel_path):
        full = self._resolve_reaper_path(rel_path)
        if not full:
            return None
        try:
            ...
            print(f"[ReaperInpaintCrop] source load error: {e}")
            return None
    ...
    def _load_disk_mask(self, rel_path):
        ...
        try:
            ...
            print(f"[ReaperInpaintCrop] mask load error: {e}")
            return None
    ...
    def run(self, size_mode="keep shape (long side)", target=1024, multiple=8,
        ...
        if state is not None:
            try:
                ...
                print(f"[ReaperInpaintCrop] state parse error: {e}")
        ...
        if image is not None:
            ...
            if src_fname:
                ui_payload = {"reaper_inpaint_source": [
        ...
        try:
            ...
            print(f"[ReaperInpaintCrop] crop error: {e}")
            return self._empty()
...
NODE_CLASS_MAPPINGS = {
    "ReaperInpaintCrop": ReaperInpaintCrop,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ReaperInpaintCrop": "Inpaint Crop Reaper",
}

__all__ = ["ReaperInpaintCrop"]
