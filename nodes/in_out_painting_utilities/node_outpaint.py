"""
Outpaint Reaper - pad an image with a solid color for an outpaint LoRA.
...
from .._helpers._rlabs_resize_helpers import _apply_max_mp, _apply_pad, _round_half_up
...
# Custom wire type carrying everything Outpaint Stitch Reaper needs to put the
...
# constant stays decoupled (no cross-file import chain), like REAPER_CROP_INFO.
...
DEFAULT_STATE = {
    ...
    "limit": 0,
    # Mid grey, not green: a LoRA trained on a green fill learns the color as
...
class ReaperOutpaint(io.ComfyNode):
    DESCRIPTION = (
        "Pads an image with a solid color so an outpainting model can fill "
        "the new area in, then optionally scales the result down to a "
        "megapixel limit and reports the final size.\n\n"
        "Mid grey is the default fill. Any color works, but a strongly "
        "colored fill can tint the whole generated image, because a model "
        "trained to replace it learns the color as well as the shape. Grey "
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="ReaperOutpaint", display_name=node_title("Outpaint"),
            ...
            outputs=[io.Image.Output(id="image", display_name="image"),
                     ...
                     OutpaintInfo.Output(id="outpaint_info", display_name="outpaint_info")],
        )

    @classmethod
    def execute(cls, **kwargs):
        return node_output(cls().outpaint(**kwargs))
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "Info for Outpaint Stitch Reaper - carries the pristine original and "
        ...
        "into Outpaint Stitch Reaper, or leave it unused.",
    )
    ...
    RETURN_TYPES = ("IMAGE", "INT", "INT", REAPER_OUTPAINT_INFO)
    ...
    def outpaint(self, image, OutpaintState=""):
        ...
        # and the per-side pads AFTER _fit_pad, so Outpaint Stitch Reaper knows
...
NODE_CLASS_MAPPINGS = {"ReaperOutpaint": ReaperOutpaint}
NODE_DISPLAY_NAME_MAPPINGS = {"ReaperOutpaint": "Outpaint Reaper"}

__all__ = ["ReaperOutpaint"]
