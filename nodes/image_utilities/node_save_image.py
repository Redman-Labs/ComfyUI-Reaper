"""Node implementation: Save Image."""
...
"""Save Image Reaper - save images to ANY folder on disk (or output/),
...
(Pattern #9). %NodeName.widget% tokens (e.g. %Seed Reaper.seed%) are
...
def _expand_native_tokens(s):
    ...
    report: they worked in Preview Image Reaper but came out literal here)."""
...
def _strip_stale_preview(extra):
    ...
    remembered preview (`redSiLastRun`) from the EMBEDDED workflow copy.

    The reasoning, and the copy-never-mutate rule that goes with it, live in
    _save_helpers.strip_stale_preview - shared with Save Video Reaper since
    2026-08-10 so the two cannot drift.
    """
    return strip_stale_preview(extra, "ReaperSaveImage", "redSiLastRun")
...
class _ReaperSaveImageEngine:
    DESCRIPTION = (
        "Save Image Reaper - save images to any folder on your computer, not just ComfyUI's output folder. "
        ...
        "from Load Image Reaper), %date:yyyy-MM-dd% (and any date/time format), %counter% (auto-incrementing, "
        "never overwrites), %width%, %height%, %batch_num%, plus node references like %Seed Reaper.seed%. "
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "name": ("STRING", {"forceInput": True, "tooltip": "Optional text used by the %input% token in the filename, e.g. wire the filename output of Load Image Reaper here to keep the original name. To save into a folder named after this text, click the + Input folder chip. If the text already contains folders they become underscores, unless you turn on 'Keep folders from the wired name' in the settings."}),
            },
            "hidden": {
    ...
    def IS_CHANGED(cls, **kwargs):
        ...
        # on day one. Same choice as Preview Image Reaper.
        return float("nan")
    ...
    def save(self, images, name=None, SaveImageState="", prompt=None, extra_pnginfo=None,
        ...
        if not _red_prescreen_field(folder_raw):
            raise ValueError(_red_denied_message(str(folder_raw)))
        folder_abs, inside_output = _resolve_save_folder(folder_raw)
        if not _red_folder_allowed(folder_abs):
            raise ValueError(_red_denied_message(folder_abs))
        ...
        if not save_on:
            ...
            return {"ui": {"reaper_save_frames": entries}}
        ...
        if civitai_on and not _metadata_disabled():
            try:
                ...
                print("[Save Image Reaper] Civitai metadata skipped: %s" % e)
        ...
        for i, tensor in enumerate(images):
            ...
            try:
                ...
                raise RuntimeError(
                    f"Save Image Reaper: cannot create folder '{target_dir}': {e}"
                )
            ...
            while True:
                ...
                try:
                    ...
                    if counter > 99999999 or suffix > 99999999:
                        raise RuntimeError(
                            "Save Image Reaper: could not find a free filename (counter overflow)"
                        )
                except OSError as e:
                    # read-only folder, AV lock, dead network share, ... -
                    # a clear message instead of a raw traceback
                    raise RuntimeError(
                        f"Save Image Reaper: cannot write in '{target_dir}': {e}"
                    )
            if has_counter:
        ...
        if note:
            status["note"] = note
        if results:
            results[0]["_reaper_status"] = status
        ...
        ui_key = "images" if inside_output else "reaper_save_frames"
        return {"ui": {ui_key: results}}

class ReaperSaveImage(io.ComfyNode):
    """Full Reaper adaptation of Save Image, powered by the imported engine."""

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="ReaperSaveImage",
            display_name=node_title('Save Image', branded=True),
            category=CATEGORIES["image_utilities"],
            description=_ReaperSaveImageEngine.DESCRIPTION,
            is_output_node=True,
            not_idempotent=True,
            inputs=[
                io.Image.Input(
                ...
                io.String.Input(
                    ...
                    optional=True,
                    force_input=True,
                    tooltip="Optional text substituted for the %input% token.",
                ),
            ],
            outputs=[],
    ...
    def execute(
        ...
        result = _ReaperSaveImageEngine().save(
            images,
            name=name,
        ...
        ui_data = result.get("ui", {}) if isinstance(result, dict) else {}
        return io.NodeOutput(ui=ui_data)

ReaperSaveImageNodes = [ReaperSaveImage]

__all__ = ["ReaperSaveImage", "ReaperSaveImageNodes"]
