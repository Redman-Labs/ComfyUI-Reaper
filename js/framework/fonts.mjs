...
const FONT_LIST_URL = "/reaper/api/fonts/list";
...
export async function getFontCatalog() {
  ...
  _catalogPromise = (async () => {
    const resp = await fetch(redApiUrl(FONT_LIST_URL), { cache: "no-store" });
...
export async function refreshFontCatalog() {
  ...
  _catalogPromise = (async () => {
    const resp = await fetch(redApiUrl(FONT_LIST_URL + "?refresh=1"), { cache: "no-store" });
...
export function urlForVariant(variant) {
  if (variant.source === "custom") {
    return redApiUrl(`/reaper/api/fonts/file/${encodeURIComponent(variant.file)}`);
  }
  return redAsset(FONT_BASE_URL + variant.file);
}
