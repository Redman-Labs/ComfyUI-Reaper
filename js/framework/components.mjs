// ╔═══════════════════════════════════════════════════════════════╗
// ║  Reaper Editor Framework — Component Factories             ║
...
export function createButton(text, opts = {}) {
  ...
  const variantClass =
    {
      standard: "rdx-btn",
      accent: "rdx-btn rdx-btn-accent",
      danger: "rdx-btn rdx-btn-danger",
      sm: "rdx-btn-sm",
      icon: "rdx-btn-icon",
      full: "rdx-btn-full",
    }[opts.variant || "standard"] || "rdx-btn";
...
export function createPanel(title, opts = {}) {
  ...
  el.className = "rdx-panel" + (opts.collapsed ? " collapsed" : "");

  const titleEl = document.createElement("div");
  titleEl.className =
    "rdx-panel-title" + (opts.collapsible ? " clickable" : "");

  if (opts.collapsible) {
    const arrow = document.createElement("span");
    arrow.className = "rdx-panel-title-arrow";
    arrow.textContent = "▼";
  ...
  content.className = "rdx-panel-content";
...
export function createSliderRow(label, min, max, value, onChange, opts = {}) {
  ...
  row.className = "rdx-slider-row";

  const lbl = document.createElement("label");
  lbl.className = "rdx-slider-label";
  ...
  function _syncFill() {
    ...
    slider.style.setProperty("--rdx-fill", ((v - mn) / (mx - mn)) * 100 + "%");
  }
  _syncFill();
...
export function createColorInput(opts = {}) {
  const input = document.createElement("input");
  input.type = "color";
  input.className = "rdx-color-input";
...
export function createRow(label, content, opts = {}) {
  const row = document.createElement("div");
  row.className = "rdx-row";

  const lbl = document.createElement("span");
  lbl.className = "rdx-row-label";
  lbl.textContent = label;
...
export function createPillGrid(options, columns, onChange, opts = {}) {
  ...
  options.forEach((opt) => {
    ...
    pill.className = "rdx-pill" + (opt.value === activeValue ? " active" : "");
...
export function createToolButton(icon, label, onClick, opts = {}) {
  const btn = document.createElement("button");
  btn.className = "rdx-tool-btn" + (opts.active ? " active" : "");
  if (opts.title) btn.title = opts.title;

  const iconEl = document.createElement("span");
  iconEl.className = "rdx-tool-btn-icon";
  iconEl.textContent = icon;

  const labelEl = document.createElement("span");
  labelEl.className = "rdx-tool-btn-label";
...
export function createZoomControls(onZoomIn, onZoomOut, onFit) {
  ...
  bar.className = "rdx-zoom-bar";

  const label = document.createElement("span");
  label.className = "rdx-zoom-label";
...
export function createTransformPanel(config) {
  ...
  if (config.onReset) {
    ...
    resetBtn.classList.add("rdx-btn-danger");
