// ╔═══════════════════════════════════════════════════════════════╗
// ║  Reaper Editor Framework — Canvas Components               ║
...
export function createCanvasSettings(config) {
  ...
  wrapper.className = "rdx-canvas-settings";

  // ── Ratio buttons ──
  const ratioGrid = document.createElement("div");
  ratioGrid.className = "rdx-ratio-grid";
  const ratioBtns = [];

  CANVAS_RATIOS.forEach((r, i) => {
    const btn = document.createElement("button");
    btn.className = "rdx-ratio-btn" + (i === curRatio ? " active" : "");
  ...
  sizeRow.className = "rdx-size-row";

  const wLabel = document.createElement("span");
  wLabel.className = "rdx-size-label";
  wLabel.textContent = "W";

  const wInput = document.createElement("input");
  wInput.type = "number";
  wInput.className = "rdx-size-input";
  wInput.value = curW;
  wInput.min = minSize;
  wInput.max = maxSize;

  const xSign = document.createElement("span");
  xSign.className = "rdx-size-x";
  xSign.textContent = "\u00d7";

  const hLabel = document.createElement("span");
  hLabel.className = "rdx-size-label";
  hLabel.textContent = "H";

  const hInput = document.createElement("input");
  hInput.type = "number";
  hInput.className = "rdx-size-input";
  hInput.value = curH;
  hInput.min = minSize;
  hInput.max = maxSize;
  ...
  const swapBtn = createButton("", {
    variant: "icon",
    iconSrc: redAsset(UI_ICON + "swap.svg"),
    onClick: () => _swap(),
...
export function createCanvasFrame(workspace) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    m.className = "rdx-canvas-mask";
    workspace.appendChild(m);
    masks.push(m);
  }

  const frame = document.createElement("div");
  frame.className = "rdx-canvas-frame";
  workspace.appendChild(frame);

  const label = document.createElement("div");
  label.className = "rdx-canvas-frame-label";
...
export function createCanvasToolbar(config) {
  ...
  const wrapper = document.createElement("div");
  wrapper.className = "rdx-canvas-toolbar";
  ...
  const addBtn = createButton(addImageLabel, {
    variant: "full",
    iconSrc: redAsset(UI_ICON + "upload.svg"),
  ...
  if (showBgColor) {
    ...
    addRow.className = "rdx-canvas-toolbar-row";
    addBtn.style.flex = "1";
    ...
    colorInput.className = "rdx-color-input";
    ...
    transpRow.className = "rdx-check-row";
  ...
  if ((showClear && onClear) || (showReset && onReset)) {
    ...
    dangerRow.className = "rdx-canvas-toolbar-row";
    ...
    if (showClear && onClear) {
      ...
      clearBtn.classList.add("rdx-btn-danger");
  ...
  function setupDropZone(workspace) {
    ...
    overlay.className = "rdx-drop-overlay";
    overlay.innerHTML = '<span class="rdx-drop-label">Drop image here</span>';
    ...
    const overlayEl = workspace.closest(".rdx-overlay");
    if (overlayEl) {
