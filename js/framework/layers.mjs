// ╔═══════════════════════════════════════════════════════════════╗
// ║  Reaper Editor Framework — Layer System                    ║
...
function _layerIcon(name, size = 12) {
  ...
  img.src = redAsset(LAYER_ICON_BASE + name + ".svg");
  img.width = size;
  img.height = size;
  img.draggable = false;
  return img;
 ...
 * Used for the "locked" state so the icon matches Reaper brand orange.
 */
function _layerIconColored(name, color, size = 12) {
  const span = document.createElement("span");
  const url = redAsset(LAYER_ICON_BASE + name + ".svg");
...
function _layerActionBtn(iconName, title, onClick, cls = "") {
  ...
  btn.className = "rdx-layer-action-btn" + (cls ? " " + cls : "");
  btn.title = title;
  // If iconName starts with "/", treat it as an already-built asset URL
  // (i.e. the output of redAsset(), e.g. redAsset("icons/3D/drop-on-floor.svg"))
  // and render it directly. Otherwise it is a bare icon NAME and we prepend the
  // default layers icon base, routing it through redAsset ourselves.
  // Never hand this a raw "/reaper/assets/..." string - that path is blocked
...
export function createLayerItem(config) {
  ...
  thumbWrap.className = "rdx-layer-thumb";
  if (config.thumbnail) thumbWrap.appendChild(config.thumbnail);

  // Name
  const nameEl = document.createElement("span");
  nameEl.className = "rdx-layer-name";
  ...
  function startRename() {
    ...
    input.className = "rdx-layer-name-input";
  ...
  if (config.onRename) {
    ...
    editBtn.className = "rdx-layer-icon";
  ...
  lock.className = "rdx-layer-icon";
  lock.title = "Toggle lock";
  let _locked = config.locked;
  const lockIcon = _locked
    ? _layerIconColored("lock-locked", "var(--rdx-accent)")
...
export function createLayersList(config) {
  ...
  list.className = "rdx-layers-list";
  panel.content.appendChild(list);

  // ── Drag-to-reorder ──
  let dragIdx = -1;
  list.addEventListener("dragstart", (e) => {
    const item = e.target.closest(".rdx-layer-item");
    if (!item) return;
    ...
    if (t.closest && t.closest(".rdx-layer-icon")) {
      e.preventDefault();
      return;
    }
  ...
  list.addEventListener("dragover", (e) => {
    ...
    list.querySelectorAll(".rdx-layer-item").forEach((el) => {
  ...
  actions.className = "rdx-layers-actions";
  if (config.onAdd)
    actions.appendChild(
      _layerActionBtn(
        config.addIcon || "add",
  ...
  if (config.onDropToFloor)
    actions.appendChild(
      _layerActionBtn(
        config.dropToFloorIcon || redAsset("icons/3D/drop-on-floor.svg"),
...
export function createLayerPanel(config) {
  ...
  wrapper.className = "rdx-layer-panel";
  ...
  if (config.showBlendMode !== false) {
    ...
    blendRow.className = "rdx-layer-blend-row";
    const defaultModes = [
    ...
    blendSelect.className = "rdx-layer-blend-select";
  ...
  if (config.showOpacity !== false) {
    ...
    opRow.className = "rdx-layer-opacity-row";
    const opLabel = document.createElement("span");
    opLabel.className = "rdx-layer-opacity-label";
    ...
    opacitySlider.style.setProperty("--rdx-fill", "100%");
  ...
  resizeHandle.className = "rdx-layers-resize";
  resizeHandle.title = "Drag to resize layer list";
  wrapper.appendChild(resizeHandle);

  // Action buttons
  const actionsEl = layersList.el.querySelector(".rdx-layers-actions");
  ...
  return {
    ...
    setOpacity(v) {
      if (opacitySlider) {
        ...
        opacitySlider.style.setProperty("--rdx-fill", v + "%");
      }
