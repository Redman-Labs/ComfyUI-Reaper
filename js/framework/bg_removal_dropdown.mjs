...
// Reaper. Both editors render an identical model picker:
...
export async function fetchBgRemovalInfo() {
  try {
    // DO NOT wrap this route in redApiUrl(). `api.fetchApi` ALREADY calls
    ...
    // hosted case redApiUrl exists for) the guard does not fire and you get
    // "/abc/comfy/api/abc/comfy/api/reaper/remove_bg_info" -> 404.
    // redApiUrl is for a BARE `fetch()`, never for `api.fetchApi`.
    const res = await api.fetchApi("/reaper/remove_bg_info", { method: "GET" });
...
export function buildBgRemovalDropdown({ container, info, value, onChange }) {
  ...
  grpBiRef.label = "Reaper BiRefNet";
  ...
  const HELP_DOCS_URL = "https://gitlab.com/reaper/comfyui-reaper#2-optional-ai-background-removal";
