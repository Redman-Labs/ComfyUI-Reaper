// ╔═══════════════════════════════════════════════════════════════╗
// ║  Reaper Shared — Constants & Utility Functions             ║
// ╚═══════════════════════════════════════════════════════════════╝

export const allow_debug = false;

export const REAPER_LOGO = `data:image/svg+xml;utf8,${encodeURIComponent(`
...
export function installFocusTrap(overlay) {
  ...
  const refocus = (e) => {
    const t = e.target;
    const tag = t?.tagName;
    // Exempt contenteditable regions (e.g. Note Reaper editor) — refocusing
...
export function restorePreview(node, widgetName, app) {
  ...
  try {
    ...
    img.src = `/view?filename=${encodeURIComponent(fn)}&type=input&subfolder=reaper&t=${Date.now()}`;
  } catch (e) {
    console.warn("[Reaper] restore failed:", e);
  }
}
...
export async function downloadDataURL(
  dataURL,
  suggestedName = "reaper_export.png",
) {
  if (!dataURL) return;
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      console.warn("[Reaper] showSaveFilePicker failed, falling back:", e);
    }
  }
