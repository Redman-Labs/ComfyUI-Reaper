// ╔═══════════════════════════════════════════════════════════════╗
// ║  Reaper Shared - Node Help panel                            ║
// ╚═══════════════════════════════════════════════════════════════╝
//
// A reusable "Help" affordance for Reaper nodes. ComfyUI's built-in Info tab
...
//   {
//     title:   "My Node Reaper",          // shown in the header
...
const CSS_ID = "rl-help-css";
const QUESTION_ICON = "/reaper/assets/icons/note/question.svg";
const BRAND = "#f66744";
...
// for whichever Reaper node is selected, without each node wiring its own
...
.rl-help-btn:hover { background-color: ${BRAND}; }
...
.rl-help-card {
  ...
  animation: rl-help-in 0.14s ease;
}
@keyframes rl-help-in {
...
.rl-help-h-title { flex: 1; font-size: 15px; font-weight: 600; color: #fff; line-height: 1.2; }
.rl-help-close {
...
.rl-help-close:hover { background: ${BRAND}; color: #fff; }

.rl-help-body { padding: 14px 16px 16px 16px; overflow-y: auto; font-size: 12.5px; line-height: 1.55; }
.rl-help-section { margin-bottom: 15px; }
.rl-help-section:last-child { margin-bottom: 0; }
.rl-help-h {
  margin: 0 0 6px 0; font-size: 11px; font-weight: 700; color: ${BRAND};
  text-transform: uppercase; letter-spacing: 0.5px;
}
.rl-help-p { margin: 0 0 6px 0; white-space: pre-wrap; color: #cfcfcf; }
.rl-help-p:last-child { margin-bottom: 0; }
.rl-help-ul { margin: 0; padding-left: 18px; }
.rl-help-ul li { margin: 0 0 4px 0; }
.rl-help-defs { display: grid; grid-template-columns: auto 1fr; gap: 5px 14px; align-items: baseline; }
.rl-help-defs dt { color: #fff; font-weight: 600; white-space: nowrap; }
.rl-help-defs dd { margin: 0; color: #bcbcbc; }
.rl-help-table { width: 100%; border-collapse: collapse; font-size: 12px; }
.rl-help-table th {
...
.rl-help-table td { padding: 5px 8px; border-bottom: 1px solid #262626; vertical-align: top; color: #cfcfcf; }
.rl-help-table tr:last-child td { border-bottom: none; }
.rl-help code {
...
.rl-help-more:hover { background: ${BRAND}; border-color: ${BRAND}; color: #fff; }
.rl-help-more:focus-visible { outline: 2px solid ${BRAND}; outline-offset: 2px; }
`;
...
function buildSection(section) {
  ...
  sec.className = "rl-help-section";

  if (section.heading) {
    const h = document.createElement("div");
    h.className = "rl-help-h";
 ...
 *
 * `target` is a comfyClass ("ReaperOutpaint") or a page key ("canvas:colors").
...
export function openHelpFor(target, helpDef, opts = {}) {
  ...
  try {
    const browser = window.ReaperHelpBrowser;
...
export function openHelpPopup(helpDef, opts = {}) {
  ...
  backdrop.className = "rl-help-backdrop";

  const card = document.createElement("div");
  card.className = "rl-help-card rl-help";
  backdrop.appendChild(card);

  // header
  const header = document.createElement("div");
  header.className = "rl-help-header";
  const icon = document.createElement("span");
  icon.className = "rl-help-h-icon";
  const title = document.createElement("div");
  title.className = "rl-help-h-title";
  title.textContent = helpDef.title || "Help";
  const close = document.createElement("button");
  close.className = "rl-help-close";
  close.type = "button";
  close.textContent = "✕";
  ...
  body.className = "rl-help-body";
  if (helpDef.tagline) {
    const tag = document.createElement("p");
    tag.className = "rl-help-p";
  ...
  for (const section of sections) {
    try {
      ...
      console.warn("Reaper help: skipped a malformed section", e);
    }
  }
  if (helpDef.footer) {
    const tip = document.createElement("div");
    tip.className = "rl-help-tip";
  ...
  if (window.ReaperHelpBrowser?.open) {
    const more = document.createElement("button");
    more.type = "button";
    more.className = "rl-help-more";
    ...
    more.addEventListener("click", (e) => {
      ...
      cleanup();
      try { window.ReaperHelpBrowser.open(cls); } catch (err) { console.warn("[Reaper] help browser", err); }
    });
