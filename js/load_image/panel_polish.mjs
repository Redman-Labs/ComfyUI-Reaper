// Per-mode panel post-processors for Load Image Reaper. Adapted from
// js/image_resize/index.js; class names use the .rl-li-* family that the
...
export function applyInlineLabel(panel, mode) {
  ...
  if (!label) return;
  panel.querySelector(".rl-li-panel-label")?.remove();
  const num = panel.querySelector(".rl-li-numinput");
  if (!num || num.querySelector(".rl-li-inline-label")) return;
  const lab = document.createElement("span");
  lab.className = "rl-li-inline-label";
  lab.textContent = label;
  num.insertBefore(lab, num.firstChild);
  num.classList.add("rl-li-num-labeled");
}
...
export function applyWHLayout(panel) {
  const fields = [...panel.querySelectorAll(".rl-li-wh-field")];
  const tags = ["W", "H"];
  fields.forEach((f, i) => {
    f.querySelector(".rl-li-wh-label")?.remove();
    const num = f.querySelector(".rl-li-numinput");
    if (num && !num.querySelector(".rl-li-inline-label")) {
      const lab = document.createElement("span");
      lab.className = "rl-li-inline-label";
  ...
  panel.querySelector(".rl-li-wh-rect-label")?.remove();

  const row = panel.querySelector(".rl-li-wh-row");
  const swap = panel.querySelector(".rl-li-swap");
  const preview = panel.querySelector(".rl-li-wh-preview");
  if (row && fields.length === 2 && preview && !panel.querySelector(".rl-li-wh-grid")) {
    const grid = document.createElement("div");
    grid.className = "rl-li-wh-grid";
    const col = document.createElement("div");
    col.className = "rl-li-wh-col";
...
export function applyCoverControls(node, panel, readState, writeState, onChange) {
  ...
  const swap = panel.querySelector(".rl-li-swap");
  if (swap && !panel.querySelector(".rl-li-fillcrop")) {
    const row = document.createElement("div");
    row.className = "rl-li-swaprow";
    const toggle = document.createElement("div");
    toggle.className = "rl-li-fillcrop";
  ...
  const preview = panel.querySelector(".rl-li-wh-preview");
  if (preview && !panel.querySelector(".rl-li-anchor")) {
    const ANCHORS = [
    ...
    const grid = document.createElement("div");
    grid.className = "rl-li-anchor";
    grid.title = "Where to crop from";
    for (const a of ANCHORS) {
      const cell = document.createElement("div");
      cell.className = "rl-li-anchor-cell" + (a === cur ? " active" : "");
    ...
    grid.addEventListener("click", (e) => {
      const cell = e.target.closest(".rl-li-anchor-cell");
      if (!cell) return;
