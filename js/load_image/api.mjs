...
export function updateNativePreview(node, filename) {
  if (!filename) return;
  node._redLiPreviewReqId = (node._redLiPreviewReqId | 0) + 1;
  const myReq = node._redLiPreviewReqId;
  const { subfolder, filename: name } = splitFilenameSubfolder(filename);
  const img = new Image();
  img.onload = () => {
    if (node._redLiPreviewReqId !== myReq) return; // stale, newer pick won
    node.imgs = [img];
    ...
    node._redLiOnImageLoaded?.();
  };
  img.onerror = () => {
    if (node._redLiPreviewReqId !== myReq) return;
    console.warn("[ReaperLoadImage] preview fetch failed for", filename);
  };
...
//   - widget.value write
//   - per-node `_redLiSelectedFilename` cache (defensive sync used by the
...
export function setSelectedImage(node, filename) {
  ...
  w.value = filename;
  node._redLiSelectedFilename = filename;
  ...
  if (!/clipspace/i.test(filename)) node._redLiOrigName = filename;
  updateNativePreview(node, filename);
  node._redLiOnFilenameChanged?.(filename);
  node.graph?.setDirtyCanvas?.(true, true);
}
...
export async function uploadImageToInput(node, file, filenameHint = null) {
  ...
  const imageWidget = node._redLiImageWidget || (node.widgets || []).find((w) => w.name === "image");
  if (imageWidget) {
    if (!node._redLiImageWidget) node._redLiImageWidget = imageWidget;
