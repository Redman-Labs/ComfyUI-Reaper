// Load Images from Folder Reaper — backend fetch helpers.
...
export async function listFolder(folder, recursive) {
  try {
    const url = redApiUrl(
      `/reaper/api/load_images_folder/list?path=${encodeURIComponent(folder)}` +
...
export function thumbURL(folder, rel, mtime) {
  return redApiUrl(
    `/reaper/api/load_images_folder/thumb?path=${encodeURIComponent(folder)}` +
...
export async function browseFolder(path) {
  try {
    const url = redApiUrl(`/reaper/api/load_images_folder/browse?path=${encodeURIComponent(path || "")}`);
...
export async function pickNativeFolder(startPath) {
  try {
    const url = redApiUrl(`/reaper/api/load_images_folder/pick_native?path=${encodeURIComponent(startPath || "")}`);
    const r = await fetch(url);
    return await r.json();
  } catch (e) {
