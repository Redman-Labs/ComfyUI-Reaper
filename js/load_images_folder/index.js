// Load Images from Folder Reaper — extension entry point.
// Mirrors Load Image Reaper's both-renderer DOM-widget + Pattern #9 state
...
} from "../shared/index.mjs?reaper=3";
import {
  installNodeAccent,
  registerNodeAccent,
} from "../shared/node_settings.mjs?reaper=3";
import { isGraphLoading } from "../shared/graph_loading.mjs?reaper=3";
import {
  COMFY_CLASS,
  HIDDEN_INPUT_NAME,
  readState,
...
import { listFolder, pickNativeFolder } from "./api.mjs?reaper=2";
import {
  injectCSS,
  buildRoot,
  openPickGallery,
  openBrowsePopup,
  openMiniMenu,
} from "./ui.mjs?reaper=3";
import { buildModePanel, injectResizePanelCSS } from "../shared/resize_panel.mjs?reaper=3";
...
function refit(node) {
  if (!node._redLifUI) return;
  requestAnimationFrame(() => {
    if (!node._redLifUI || isGraphLoading()) return;
...
function renderUI(node) {
  ...
  ui.msgEl.textContent = node._redLifListError || "";
  node.setDirtyCanvas?.(true, true);
}
...
async function refreshListing(node, userAction = false) {
  ...
  const myReq = (node._redLifListReq = (node._redLifListReq || 0) + 1);
  const state = readState(node);
  if (!state.folder) {
    node._redLifFiles = [];
    node._redLifListError = "";
    renderUI(node);
    return;
  }
  ...
  if (node._redLifListReq !== myReq || !node._redLifUI) return;
  if (res && res.ok) {
    node._redLifFiles = res.files || [];
    node._redLifListError = node._redLifFiles.length
      ? ""
      : "No images found in this folder.";
  } else {
    node._redLifFiles = [];
    node._redLifListError = (res && res.message) || "Folder not found.";
  }
  ...
  const present = new Set((node._redLifFiles || []).map((f) => f.file));
...
function renderResize(node) {
  ...
  btn.type = "button";
  btn.className = "red-lif-resizebtn";
...
function setupNode(node) {
  ...
  const widget = node.addDOMWidget("reaper_lif_ui", "custom", ui.root, {
    getValue: () => null,
    setValue: () => {},
  ...
  ui.pickBtn.addEventListener("click", async () => {
    ...
    if (!st.folder) {
      ...
      node._redLifListError = "Set a folder first (type, paste, or Browse).";
      renderUI(node);
      return;
    }
  ...
  try {
    node._redLifFloorOff = installResizeFloor(ui.root, measureContentHeight);
  } catch {}
...
function installGraphToPromptHook() {
  if (app._redLifGraphPatched) return;
  app._redLifGraphPatched = true;
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onRemoved = function () {
      ...
      this._redLifGallery?._redClose?.();
      this._redLifBrowsePop?._redClose?.();
      document.querySelectorAll(".red-lif-menu").forEach((p) => p._redClose?.());
      return origRemoved?.apply(this, arguments);
    };
  },
});

// The color option: a right-click "Load Images from Folder settings" entry, the gear in the
// selection toolbar, and the shared color panel behind both.
registerNodeAccent("ReaperLoadImagesFolder", { title: "Load Images from Folder" });
