...
// Outpaint Stitch Reaper - the two native INT widgets (feather, color_match)
// become Sliders-Reaper-style sliders, EACH with a real input dot on its row
...
app.registerExtension({
  name: "Reaper.OutpaintStitch",
  // No Settings-panel row: this node's color lives in its OWN panel (the
  ...
  // "Every Reaper node" could never reach this node. Unregistered, the key
  // is written ONLY by an explicit "New ... nodes" press, so stored == chosen.

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== CLASS) return;
    if (nodeType.prototype._redOpsPatched) return;
    nodeType.prototype._redOpsPatched = true;
...
app.graphToPrompt = async function (...args) {
  ...
  try {
    ...
    console.warn("[Outpaint Stitch Reaper] could not inject slider values:", (e && e.message) || e);
  }
  return result;
};
