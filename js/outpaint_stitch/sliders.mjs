// Outpaint Stitch Reaper - the slider face. Two recess-style sliders (Feather,
// Color Match) matching Sliders Reaper's look, bound to the node's native INT
...
import { BRAND, widgetOf, accentOf } from "./core.mjs";
...
const WIDGET_TYPE = "reaper_ops_row";
...
export function injectCSS() {
  if (document.getElementById("red-ops-css")) return;
  const s = document.createElement("style");
  s.id = "red-ops-css";
  s.textContent = `
    .red-ops-row-w { width:100%; box-sizing:border-box; }
    /* Heights must be DEFINITE (Sliders Pattern #2) or the min-content Vue row
       collapses (content is absolutely positioned). */
    .red-ops-row { position:relative; width:100%; height:${ROW_H}px; min-height:${ROW_H}px; box-sizing:border-box; }
    ...
    .red-ops-sl:hover { border-color:var(--acc,${BRAND}); }
    ...
    .red-ops-sl.wired { cursor:default; opacity:0.5; filter:grayscale(0.9); }
    .red-ops-sl.wired:hover { border-color:rgba(255,255,255,0.14); }
    .red-ops-fill { position:absolute; left:0; top:0; bottom:0; width:0; background:var(--acc,${BRAND}); }
    .red-ops-lay {
    ...
    .red-ops-lay .nm { flex:1; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .red-ops-lay .nu { flex:none; font-weight:600; font-variant-numeric:tabular-nums; }
    .red-ops-base { clip-path:inset(0 0 0 var(--p,0%)); }
    .red-ops-over { clip-path:inset(0 calc(100% - var(--p,0%)) 0 0); }
    .red-ops-base .nm { color:rgba(255,255,255,0.72); }
    .red-ops-base .nu { color:var(--acc,${BRAND}); }
    .red-ops-over .nm, .red-ops-over .nu { color:#fff; }
    .red-ops-edit {
      position:absolute; inset:0; width:100%; height:100%; box-sizing:border-box; display:none;
      background:#1d1d1d; border:1px solid var(--acc,${BRAND}); border-radius:5px; outline:none;
      color:#e8e8e8; font:11.5px 'Segoe UI',sans-serif; text-align:right; padding:0 8px;
      font-variant-numeric:tabular-nums;
    }
    .red-ops-sl.editing .red-ops-edit { display:block; }
    ...
    .lg-node-widget:has(.red-ops-row) > div:first-child { opacity:1 !important; }
  `;
...
function makeSliderRow(node, cfg) {
  ...
  wrap.className = "red-ops-row-w";
  const row = document.createElement("div");
  row.className = "red-ops-row";
  const sl = document.createElement("div");
  sl.className = "red-ops-sl";
  const fill = document.createElement("div");
  fill.className = "red-ops-fill";
  const base = document.createElement("div");
  base.className = "red-ops-lay red-ops-base";
  base.innerHTML = '<span class="nm"></span><span class="nu"></span>';
  const over = document.createElement("div");
  over.className = "red-ops-lay red-ops-over";
  over.innerHTML = '<span class="nm"></span><span class="nu"></span>';
  const edit = document.createElement("input");
  edit.className = "red-ops-edit";
  edit.type = "text";
...
export function paintRows(node) {
  ...
  for (const wrap of wraps) {
    ...
    wrap.querySelector(".red-ops-fill").style.width = p + "%";
    wrap.querySelectorAll(".red-ops-lay").forEach((lay) => {
...
export function bindInputDots(node) {
  ...
  for (const cfg of SLIDERS) {
    ...
    const w = node._redOpsRowWidgets?.[cfg.name];
    if (!inp || !w) continue;
    if (vue) {
...
export function alignInputsLegacy(node) {
  if (isVueNodes() || !node._redOpsRowWidgets) return;
  for (const cfg of SLIDERS) {
    const inp = node.inputs?.find((i) => i.name === cfg.name);
    const w = node._redOpsRowWidgets[cfg.name];
...
export function installSliders(node) {
  if (node._redOpsWraps) { paintRows(node); return; }
  for (const cfg of SLIDERS) hideNativeWidget(widgetOf(node, cfg.name));

  node._redOpsWraps = [];
  node._redOpsRowWidgets = {};
  node._redOpsFloorOffs = [];
  for (const cfg of SLIDERS) {
    ...
    node._redOpsWraps.push(wrap);
    node._redOpsRowWidgets[cfg.name] = w;
    ...
    node._redOpsFloorOffs.push(installResizeFloor(wrap, (root) => root.offsetHeight || ROW_H));
...
export function uninstallSliders(node) {
  try { (node._redOpsFloorOffs || []).forEach((off) => off?.()); } catch {}
  node._redOpsFloorOffs = [];
}
