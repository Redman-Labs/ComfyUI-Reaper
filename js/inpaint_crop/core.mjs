// ============================================================
// Inpaint Crop Reaper — Editor core (constructor, open/close, build UI)
// ============================================================
import {
  BRAND,
  createEditorLayout,
...
import { redConfirm } from "../shared/confirm_dialog.mjs";
...
export const InpaintAPI = {
  async uploadSrc(projectId, dataURL) {
    ...
    const res = await api.fetchApi(redApiUrl("/reaper/api/inpaint/upload_src"), {
  ...
  async saveMask(projectId, dataURL) {
    const { api } = await import("/scripts/api.js");
    const res = await api.fetchApi(redApiUrl("/reaper/api/inpaint/save_mask"), {
      method: "POST", headers: { "Content-Type": "application/json" },
...
export class InpaintCropEditor {
  ...
  open(jsonStr, upstreamUrl, params, prefs) {
    ...
    if (upstreamUrl) sourceURL = upstreamUrl;
      ...
      sourceURL = redApiUrl(`/view?filename=${encodeURIComponent(fn)}&type=input&subfolder=reaper&t=${Date.now()}`);
    }
    ...
    if (sourceURL) {
      this._loadImageFromURL(sourceURL, () => {
        ...
        if (this._maskPath) {
          ...
          const murl = redApiUrl(`/view?filename=${encodeURIComponent(mfn)}&type=input&subfolder=reaper&t=${Date.now()}`);
  ...
  _buildUI() {
    const layout = createEditorLayout({
      ...
      editorId: "reaper-inpaint-editor",
      showUndoRedo: true,
      showZoomBar: false,
      showStatusBar: true,
      onSave: () => this._save(),
      ...
      helpContent: `
        <b style="color:${BRAND};">WHAT THIS DOES</b><br>
        ...
        <b style="color:${BRAND};">PAINTING</b><br>
        ...
        <b style="color:${BRAND};">CROP SIZE</b> (sidebar + node)<br>
        ...
        <b style="color:${BRAND};">SEAM — HOW IT BLENDS BACK</b><br>
  ...
  _buildLeftSidebar(sidebar) {
    ...
    row.append(
      createButton("Invert", { variant: "standard", iconSrc: redAsset(UI + "swap.svg"), onClick: () => this._invertMask() }),
      createButton("Clear", { variant: "standard", iconSrc: redAsset(UI + "delete.svg"), onClick: () => this._clearMask() }),
    );
    ...
    this._visBtn = createButton("Toggle mask (H)", { variant: "full", iconSrc: redAsset(UI + "eraser.svg"), onClick: () => this._toggleMaskVisible() });
    ...
    const resetAll = createButton("Reset all to default", {
      variant: "standard", iconSrc: redAsset(UI + "reset.svg"), onClick: () => this._resetAll(),
    });
    ...
    const loadBtn = createButton("Load Image", { variant: "full", iconSrc: redAsset(UI + "image.svg"), onClick: () => fileInput.click() });
