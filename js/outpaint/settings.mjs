// Outpaint Reaper - the floating settings panel (Sizes / Run Timer / Save Image
...
// the per-node accent color, and the final-size snap. The node face stays
...
let _cpHandle = null;  // the open color picker, so the panel can close it too
...
function injectCSS() {
  if (document.getElementById("red-opp-css")) return;
  ...
  s.textContent = `
    .red-opp { position:fixed; z-index:10010; width:300px; max-width:94vw; background:#1a1a1a;
    ...
    .red-opp-t { display:flex; align-items:center; gap:8px; padding:10px 12px; background:#232323;
      border-bottom:1px solid #333; cursor:grab; user-select:none; color:var(--acc,${BRAND}); }
    .red-opp-t .x { margin-left:auto; color:#8a8a8a; cursor:pointer; padding:0 4px; }
    .red-opp-t .x:hover { color:#fff; }
    .red-opp-b { padding:12px; display:flex; flex-direction:column; gap:14px; max-height:64vh; overflow-y:auto; }

    .red-opp-field { display:flex; flex-direction:column; gap:6px; }
    .red-opp-lab { font-size:12px; color:#9a9a9a; display:flex; align-items:baseline; gap:6px; }
    .red-opp-lab .cnt { color:var(--acc,${BRAND}); font-variant-numeric:tabular-nums; }
    ...
    .red-opp-grid { display:grid; grid-template-columns:repeat(6,1fr); gap:5px; }
    .red-opp-chip { box-sizing:border-box; text-align:center; padding:6px 2px; border-radius:5px;
    ...
    .red-opp-chip:hover { border-color:var(--acc,${BRAND}); color:#ddd; }
    .red-opp-chip.on { background:var(--acc,${BRAND}); border-color:var(--acc,${BRAND}); color:#fff; }
    /* A chip that cannot be toggled right now (the last one on, or a 7th) still
       reads as a choice, just not an available one. */
    .red-opp-chip.locked { opacity:.5; cursor:default; }
    .red-opp-chip.locked:hover { border-color:#444; color:#aaa; }
    .red-opp-chip.on.locked:hover { border-color:var(--acc,${BRAND}); color:#fff; }

    .red-opp-msg { font-size:11px; color:var(--acc,${BRAND}); min-height:14px;
      opacity:0; transition:opacity .12s; }
    .red-opp-msg.show { opacity:1; }
    ...
    .red-opp-mrow { display:flex; flex-wrap:wrap; gap:5px; }
    .red-opp-mchip { display:inline-flex; align-items:center; gap:5px; box-sizing:border-box;
    ...
    .red-opp-mchip .mx { color:#8a8a8a; cursor:pointer; font-size:12px; line-height:1;
      padding:0 1px; border-radius:3px; }
    .red-opp-mchip .mx:hover { color:#fff; background:#e0604a; }
    .red-opp-add { display:flex; align-items:center; gap:8px; }
    .red-opp-add input { flex:1; min-width:0; box-sizing:border-box; background:#161616;
    ...
    .red-opp-add input:focus { border-color:var(--acc,${BRAND}); }
    .red-opp-add .unit { flex:0 0 auto; color:#8a8a8a; font-size:11px; }
    .red-opp-addbtn { flex:0 0 auto; background:var(--acc,${BRAND}); color:#fff; border:0;
      border-radius:6px; padding:8px 14px; font:12px 'Segoe UI',sans-serif; cursor:pointer; }
    .red-opp-addbtn:hover { filter:brightness(1.08); }
    .red-opp-addbtn:disabled { opacity:.4; cursor:default; filter:none; }

    .red-opp-seg { display:flex; gap:4px; }
    .red-opp-seg button { flex:1; text-align:center; padding:6px 2px; border-radius:5px;
    ...
    .red-opp-seg button:hover { color:#ddd; }
    .red-opp-seg button.on { background:var(--acc,${BRAND}); border-color:var(--acc,${BRAND}); color:#fff; font-weight:600; }

    .red-opp-acc { display:flex; align-items:center; gap:10px; }
    .red-opp-sw { width:30px; height:22px; border-radius:5px; border:1px solid #555; cursor:pointer; flex:none; }
    .red-opp-sw:hover { border-color:#fff; }

    .red-opp-f { display:flex; gap:8px; align-items:center; padding:10px 12px; border-top:1px solid #333; background:#1f1f1f; }
    .red-opp-btn { border:1px solid #444; background:rgba(255,255,255,0.04); color:#d8d8d8; border-radius:5px;
      padding:6px 12px; font:12px 'Segoe UI',sans-serif; cursor:pointer; }
    .red-opp-btn:hover { border-color:var(--acc,${BRAND}); color:#fff; }
    .red-opp-push { margin-left:auto; }
  `;
...
function escClose(e) {
  if (e.key === "Escape" && _panel) {
    if (document.querySelector(".red-cp-popup, .red-cp-modal-backdrop")) return; // let the picker take it
    e.stopPropagation();
    closeOutpaintSettings();
  }
}
...
export function openOutpaintSettings(node, ctx) {
  ...
  function buildBody() {
    ...
    // ── ratio chips ─────────────────────────────────────────────────────────
    const rf = el("div", "red-opp-field");
    const lab = el("div", "red-opp-lab");
    lab.append(el("span", null, "Ratio buttons"),
               el("span", "cnt", `${chosen.length} of ${MAX_RATIOS}`));
    rf.appendChild(lab);

    const grid = el("div", "red-opp-grid");
    for (const r of RATIO_LIBRARY) {
      ...
      const c = el("div", "red-opp-chip" + (on ? " on" : "") + (locked ? " locked" : ""), r);
      c.title = on
    ...
    const addRow = el("div", "red-opp-add");
    const inp = el("input");
    inp.type = "text";
    ...
    const addBtn = el("button", "red-opp-addbtn", "Add");
    ...
    mMsgEl = el("div", "red-opp-msg");
    mf.appendChild(mMsgEl);
    body.appendChild(mf);

    // ── accent ──────────────────────────────────────────────────────────────
    const af = el("div", "red-opp-field");
    af.appendChild(el("div", "red-opp-lab", "Button color"));
    const acc = el("div", "red-opp-acc");
    acc.append(sw, el("div", "red-opp-lab", "The accent for this node"));
    af.appendChild(acc);
    body.appendChild(af);

    // ── snap ────────────────────────────────────────────────────────────────
    const sf = el("div", "red-opp-field");
    sf.appendChild(el("div", "red-opp-lab", "Snap the final size to a multiple of"));
    const seg = el("div", "red-opp-seg");
  ...
  sw.addEventListener("click", () => {
    ...
    // swatches keep white button text readable - REAPER_PALETTE's pale ones do
    // not). No transparent tile: an accent is always a color. onPick fires on
    // every change, so the node recolors live as the user drags. The picker
    ...
    _cpHandle = openReaperColorPickerPopup(sw, {
      ...
      wide: true,
      resetColor: BRAND, // Reset -> the Reaper orange
      onPick: (c) => {
        writeState(node, { accent: c || BRAND });
        repaintAccent();
        fire(); // recolor the node's chips + cards live
      },
    });
  });
  ...
  const mkDefault = el("button", "red-opp-btn", "Color as default");
  mkDefault.title = "Use this color for every new Outpaint node";
  ...
  const done = el("button", "red-opp-btn red-opp-push", "Done");
  ...
  const mkAll = el("button", "red-opp-btn", "Every Reaper node");
