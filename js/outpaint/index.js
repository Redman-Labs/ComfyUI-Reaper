// Outpaint Reaper - the node face: mode, ratio, add-space and limit rows, and
...
import { redApiUrl, redAsset } from "../shared/api_url.mjs";
...
import { openReaperColorPickerPopup, REAPER_PALETTE } from "../shared/color_picker.mjs";
...
// The node's accent: its own saved color, else the global default, else BRAND.
...
function accentOf(node) {
  ...
  return globalAccent() || BRAND;   // then the master Reaper default
}

const CLASS = "ReaperOutpaint";
...
const PAD = 9;      // .red-op-inner padding, top + bottom
const ROW_GAP = 6;  // gap between rows
// The preview height the floor ASKS for. Deliberately not a CSS min-height (see
// .red-op-prev): the preview may shrink below this when the body is tight, which
...
// fixed color cannot work: #0a3d0a was tuned for bright green and would be
...
// for any color the user chooses.
...
function watchSource(node) {
  clearInterval(node._redOpPoll);
  node._redOpSrcSig = sourceSig(node);
  node._redOpPoll = setInterval(() => {
    if (!node.graph) {
      clearInterval(node._redOpPoll);
      node._redOpPoll = null;
      return;
    }
    const sig = sourceSig(node);
    if (sig === node._redOpSrcSig) return;
...
function injectCSS() {
  if (document.getElementById("reaper-outpaint-css")) return;
  const css = `
    .red-op-root { position:relative; width:100%; height:100%; box-sizing:border-box;
    ...
    .red-op-inner { position:absolute; inset:0; box-sizing:border-box;
      display:flex; flex-direction:column; gap:${ROW_GAP}px; padding:${PAD}px;
      user-select:none; }
    .red-op-row { display:flex; align-items:stretch; gap:5px; flex:0 0 auto;
      flex-wrap:wrap; }
    ...
    .red-op-chip { flex:1 1 auto; min-width:0; box-sizing:border-box;
    ...
    .red-op-chip:hover { border-color:var(--red-op-acc,${BRAND}); color:#ddd; }
    .red-op-chip.on { background:var(--red-op-acc,${BRAND});
      border-color:var(--red-op-acc,${BRAND}); color:#fff; }
    /* Nothing to click: no pointer, no hover promise. */
    .red-op-chip.dim { opacity:.4; cursor:default; }
    .red-op-chip.dim:hover { border-color:#444; color:#aaa; }
    .red-op-chip.dim.on:hover { border-color:var(--red-op-acc,${BRAND}); color:#fff; }
    ...
    /* Chevron and gear: fixed, so the mode chips get every spare pixel. The
       ...
       Reaper, so the settings button reads the same across the suite. */
    .red-op-sq { flex:0 0 auto; width:30px; padding:6px 0; font-size:14px; line-height:1; }
    .red-op-alabel { flex:0 0 auto; display:flex; align-items:center;
    ...
    /* min-height matches a chip row (the fields' own content is only ~16px, and
       .red-op-row stretches, so without this the whole row reads as a thin
       cramped strip against the 28px rows above and below it). */
    .red-op-pad { flex:1 1 0; min-width:44px; min-height:28px; box-sizing:border-box;
      display:flex; align-items:center; gap:3px;
      background:#1d1d1d; border:1px solid #444; border-radius:5px;
      padding:0 4px 0 6px; }
    .red-op-pad:focus-within { border-color:var(--red-op-acc,${BRAND}); }
    .red-op-pad-l { flex:0 0 auto; font-size:9px; font-weight:700; color:#8a8a8a;
      letter-spacing:.5px; pointer-events:none; }
    /* Strip the shared wrapper's own box so only .red-op-pad draws one. */
    .red-op-pad .red-li-numinput { flex:1 1 auto; min-width:0;
      border:none !important; background:transparent !important;
      border-radius:0 !important; }
    .red-op-pad .red-li-spin { display:none !important; }
    /* user-select is none on .red-op-inner and inherits, which would stop the
       user drag-selecting the number they are about to replace. */
    .red-op-pad .red-li-numinput input { padding:0 !important;
      text-align:right !important; color:var(--red-op-acc,${BRAND});
      user-select:text; }
    ...
    .red-op-reset { flex:0 0 auto; width:26px; min-height:28px; box-sizing:border-box;
      display:flex; align-items:center; justify-content:center;
      background:#1d1d1d; border:1px solid #444; border-radius:5px;
      color:#aaa; cursor:pointer; padding:0; font:inherit;
      transition:border-color .08s, color .08s; }
    .red-op-reset:hover:not(:disabled) { border-color:var(--red-op-acc,${BRAND});
      color:var(--red-op-acc,${BRAND}); }
    .red-op-reset:focus-visible { border-color:var(--red-op-acc,${BRAND});
      color:var(--red-op-acc,${BRAND}); outline:none; }
    /* Nothing to reset: genuinely inert, not merely faded. */
    .red-op-reset:disabled { opacity:.4; cursor:default; }
    /* A mask, not a text glyph: no font-fallback tofu risk, and it matches the
       Reset on the shared pad panel. */
    .red-op-reset-ic { width:12px; height:12px; background-color:currentColor;
      -webkit-mask: url("${redAsset("icons/ui/reset.svg")}") center/12px 12px no-repeat;
              mask: url("${redAsset("icons/ui/reset.svg")}") center/12px 12px no-repeat; }

    /* The fill-color swatch. Clickable on the limit row (opens the fill
       picker); a plain readout in the folded summary. */
    .red-op-swatch { flex:0 0 auto; width:26px; border-radius:5px;
      border:1px solid #444; cursor:default; }
    .red-op-swatch-btn { cursor:pointer; }
    .red-op-swatch-btn:hover { border-color:var(--red-op-acc,${BRAND}); }
    ...
    .red-op-cards { flex:0 0 auto; width:100%; height:${CARDS_H}px; display:block; }
    ...
    .red-op-prev { position:relative; flex:1 1 0; min-height:0;
    ...
    .red-op-prev canvas { position:absolute; inset:0; width:100%; height:100%;
      display:block; }
  `;
  const s = document.createElement("style");
  s.id = "reaper-outpaint-css";
  s.textContent = css;
...
function chip(text, on, title) {
  ...
  el.className = "red-op-chip" + (on ? " on" : "");
  el.textContent = text;
...
function renderModeRow(node, host) {
  ...
  chevron.classList.add("red-op-sq");
  ...
  if (folded) {
    ...
    sw.className = "red-op-swatch";
    sw.style.background = st.color;
    sw.title = "Fill color: " + st.color;
    host.appendChild(sw);
  } else {
  ...
  const gear = chip("⚙", false, "Choose ratios and the accent color");
  gear.classList.add("red-op-sq");
...
// an accent change recolors everything).
...
function focusedPadInput(node) {
  const inputs = node._redOpPadInputs;
...
function refreshResetState(node) {
  const ui = node._redOpUI;
  const btn = ui && ui.inner.querySelector(".red-op-reset");
  if (!btn) return;
  const st = readState(node);
...
function renderLimitRow(node, host) {
  ...
  // The fill color, and the one control that changes it. This is the color the
  // outpaint model repaints, NOT the accent - a picker here is deliberately
  // separate from the settings panel's Button color, and uses the full palette
  // (neutrals AND vibrants) because a LoRA might want pure green, white or black.
  const sw = document.createElement("div");
  sw.className = "red-op-swatch red-op-swatch-btn";
  sw.style.background = st.color;
  sw.title = "Fill color (click to change): " + st.color;
...
function openFillPicker(node, anchor) {
  try { node._redOpFillPicker?.close(); } catch (_e) { /* already gone */ }
  node._redOpFillPicker = openReaperColorPickerPopup(anchor, {
    initialColor: readState(node).color,
    swatches: REAPER_PALETTE,
    wide: true,
    resetColor: DEFAULT_STATE.color, // #808080
    onPick: (c) => {
      writeState(node, { color: c || DEFAULT_STATE.color });
      renderFace(node); // recolors the swatch AND the preview bands + ink
      node.setDirtyCanvas?.(true, true);
    },
  });
}
...
function pill(ctx, text, cx, cyMid) {
  ...
  // has nothing to do with the fill color and must not carry its hue.
...
function requestPreviewRedraw(node) {
  if (node._redOpRaf) return;
  node._redOpRaf = requestAnimationFrame(() => {
    node._redOpRaf = null;
    renderPreview(node);
...
function installDrag(node) {
  ...
  const end = (e) => {
    if (!node._redOpDrag) return;
    node._redOpDrag = null;
  ...
  prev.addEventListener("pointerleave", () => {
    if (!node._redOpDrag) prev.style.cursor = "";
  });
}
...
function renderFace(node) {
  ...
  inner.style.setProperty("--red-op-acc", accentOf(node)); // per-node, from settings
...
function snapFresh(node, tries = 0) {
  requestAnimationFrame(() => {
    if (!node.graph || node._redOpConfigured || isGraphLoading()) return;
    const ui = node._redOpUI;
...
function setupNode(node) {
  ...
  root.className = "red-op-root";
  const inner = document.createElement("div");
  inner.className = "red-op-inner";
  ...
  cards.className = "red-op-cards";
  cards.style.display = isVueNodes() ? "block" : "none";
  inner.appendChild(cards);

  const prev = document.createElement("div");
  prev.className = "red-op-prev";
  const canvas = document.createElement("canvas");
  prev.appendChild(canvas);
  inner.appendChild(prev);

  node._redOpUI = { root, inner, cards, prev, canvas, _floorCache: floorFallback() };
  ...
  node._redOpRO = new ResizeObserver(repaintCanvases);
  node._redOpRO.observe(prev);
  ...
  node._redOpZoomOff = installZoomRepaint(
    node, () => [prev.clientWidth, prev.clientHeight], repaintCanvases, "_redOpZoomRaf");
  ...
  const w = node.addDOMWidget("outpaint_ui", "reaper_outpaint", root, {
    serialize: false,
...
app.registerExtension({
  name: "Reaper.Outpaint",

  // No Settings-panel row: this node's color lives in its OWN panel (the gear /
  ...
  // default permanently outranked the master accent and "Every Reaper node"
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    if (nodeType.prototype._redOpPatched) return; // hot-reload guard
    nodeType.prototype._redOpPatched = true;

    injectCSS();
    injectResizePanelCSS(); // the .red-li-* base for the shared numeric fields
    ...
    // would simply never show there) - the .red-op-cards canvas carries the same
    // pixels instead.
    ...
    nodeType.prototype.onRemoved = function () {
      ...
      clearInterval(this._redOpPoll);
      this._redOpPoll = null;
      try { this._redOpRO?.disconnect(); } catch (_e) { /* already gone */ }
      this._redOpRO = null;
      this._redOpZoomOff?.();
      this._redOpZoomOff = null;
      if (this._redOpRaf) cancelAnimationFrame(this._redOpRaf);
      this._redOpRaf = null;
      this._redOpDrag = null;
      this._redOpPadInputs = null;
      ...
      try { this._redOpFillPicker?.close(); } catch (_e) { /* already gone */ }
      this._redOpFillPicker = null;
...
if (!app._redOpExecPatched) {
  app._redOpExecPatched = true;   // hot-reload guard: one listener, not one per load
  api.addEventListener("executed", ({ detail }) => {
    try {
      const entry = detail?.output?.reaper_outpaint_base?.[0];
      ...
      img.onload = () => {
        ...
        node._redOpBaseImg = img;
        // Keep the watcher in step, or it repaints the same picture again 400ms
        // later for nothing.
        node._redOpSrcSig = sourceSig(node);
        renderFace(node);
      };
      img.src = redApiUrl("/view?filename=" + encodeURIComponent(entry.filename) +
      ...
      console.warn("[Outpaint Reaper] base preview failed:", (e && e.message) || e);
    }
  });
}
...
if (!app._redOpPromptPatched) {
  app._redOpPromptPatched = true;
  ...
  app.graphToPrompt = async function (...args) {
    ...
    try {
      ...
      console.warn("[Outpaint Reaper] could not inject state:", (e && e.message) || e);
    }
    return result;
  };
}
