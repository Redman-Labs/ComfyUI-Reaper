...
import {
  buildPauseTextWidget, renderPause, resetSearch, syncText, flashIcon, statusText,
  NODE_MIN_W, nodeMinH,
} from "./ui.mjs";

const CLASS = "ReaperPauseText";
const HIDDEN_INPUT = "PauseState";
const WIDGET_TYPE = "reaper_pause_text_ui";
...
// band - it can NEVER overflow past the node edge on resize. Some Nodes 2.0
// versions also invoke onDrawForeground, so check the real DOM-band visibility
// instead of assuming the renderer flag makes the two paths mutually exclusive.
function paintStatus(node, ctx) {
  if (node.flags?.collapsed) return;
  const band = node._redPtEls?.band;
  if (band && band.style.display !== "none") return;
...
function positionBand(node) {
  const band = node._redPtEls?.band;
  const root = node._redPtEls?.root;
  const box = node._redPtEls?.box;
...
function watchBand(node) {
  ...
  if (isVueNodes() && !node._redPtBandPoll) {
    node._redPtBandPoll = setInterval(() => {
      if (!node.graph) { clearInterval(node._redPtBandPoll); node._redPtBandPoll = null; return; }
      positionBand(node);
    }, 350);
  }
}
...
async function queueWithMode(node, mode) {
  ...
  node._redPtSubmitMode = mode;
  node._redPtBusy = mode === "continue" ? "Continuing…" : "Regenerating…";
  renderPause(node);
  try {
    ...
    node._redPtSubmitMode = null;
    node._redPtBusy = null;
    renderPause(node);
  }
}
...
function flash(node, msg) {
  node._redPtFlash = msg;
  renderPause(node);
  clearTimeout(node._redPtFlashTimer);
  node._redPtFlashTimer = setTimeout(() => {
    node._redPtFlash = null;
    renderPause(node);
  }, 2500);
}
...
async function copyText(node) {
  ...
  try {
    ...
    flashIcon(node._redPtEls?.copyBtn);
...
function setupNode(node) {
  const root = buildPauseTextWidget(node, {
    ...
    onRevert: () => { revertText(node); syncText(node); renderPause(node); flashIcon(node._redPtEls?.revertBtn); },
  });
  ...
  node._redPtFloorOff = installResizeFloor(root, () => nodeMinH(isVueNodes()), () => {
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onRemoved = function () {
      clearTimeout(this._redPtFlashTimer);
      if (this._redPtBandPoll) { clearInterval(this._redPtBandPoll); this._redPtBandPoll = null; }
      try { this._redPtFloorOff?.(); } catch { /* ignore */ }
      this._redPtFloorOff = null;
      this._redPtSearchRO?.disconnect?.();
      this._redPtSearchRO = null;
      this._redPtEls = null;
...
// Python emits reaper_pause_text ONLY on a wired Pause/Pass (a fresh model
// capture), so receiving it always means "replace the box with fresh text".
api.addEventListener("executed", (e) => {
  const d = e.detail;
  const payload = d?.output?.reaper_pause_text;
  ...
  resetSearch(node);          // a new upstream payload starts with a clean search
  syncText(node);
  renderPause(node);
});
...
function editedTextOf(node) {
  const live = node._redPtEls?.ta?.value;
...
app.graphToPrompt = async function (...args) {
  ...
  if (out) {
    ...
    for (const id in out) {
      ...
      const submit = node?._redPtSubmitMode;
      let mode;
