...
// ╚══════════════════════════════════════════════════════════════════════╝
//
// NEVER write a root-relative API URL (`/view?...`, `fetch("/reaper/api/x")`)
...
/**
 * Absolute-safe URL for a ComfyUI or Reaper route.
 * @param {string} route e.g. "/view?filename=x.png&type=output" or "/reaper/api/version"
 * @returns {string} the same route, prefixed for this deployment
 */
export function redApiUrl(route) {
  try {
 ...
 *    (verified: "/reaper/api/version?name=icons/ui/play.svg" answers 200).
 *    The server exposes bundled Reaper assets through its asset route.
 *
 ...
 */
export function redAsset(tail) {
  const rel = String(tail).replace(/^\/+/, "");
  return redApiUrl("/reaper/api/asset?path=" + encodeURIComponent(rel));
}
