...
//
// For any Reaper box whose contract is "one value per line" (XY Plot's Full
...
function injectCSS() {
  ...
  s.id = CSS_ID;
  s.textContent = `
.rl-ln-wrap{position:relative;display:block;}
...
.rl-ln-wrap > textarea{display:block;}
...
.rl-ln-gutter{position:absolute;left:0;top:0;bottom:0;overflow:hidden;pointer-events:none;user-select:none;
  border-right:1px solid rgba(255,255,255,.10);background:rgba(0,0,0,.22);border-radius:5px 0 0 5px;}
.rl-ln-inner{position:absolute;left:0;top:0;right:0;will-change:transform;}
.rl-ln-num{position:absolute;right:5px;color:#6f6f6f;font-variant-numeric:tabular-nums;}
/* The measuring twin. Never shown; must never affect layout. */
.rl-ln-mirror{position:absolute;visibility:hidden;pointer-events:none;top:0;left:-99999px;
...
export function attachLineNumbers(ta, opts = {}) {
  ...
  if (!ta) return () => {};
  if (ta._redLnDetach) return ta._redLnDetach;
  injectCSS();
  ...
  wrap.className = "rl-ln-wrap";
  parent.insertBefore(wrap, ta);
  wrap.appendChild(ta);

  const gutter = document.createElement("div");
  gutter.className = "rl-ln-gutter";
  const inner = document.createElement("div");
  inner.className = "rl-ln-inner";
  gutter.appendChild(inner);
  wrap.appendChild(gutter);

  const mirror = document.createElement("div");
  mirror.className = "rl-ln-mirror";
  ...
  const detach = () => {
    ...
    delete ta._redLnDetach;
  };
  ta._redLnDetach = detach;
  return detach;
}
