// ╔═══════════════════════════════════════════════════════════════╗
// ║  Reaper shared font picker popup                            ║
...
// (.red-to-popup*) so nothing about the Text Overlay / Watermark / Composer
...
export function openFontPopup(anchorEl, opts) {
  ...
  injectFontPickerCSS();
  document.querySelector(".red-to-popup")?.remove();
  const popup = document.createElement("div");
  popup.className = "red-to-popup";
  ...
  searchRow.className = "red-to-popup-search";
  const mag = document.createElement("span");
  mag.className = "red-to-popup-mag";
  mag.textContent = "⌕";
  ...
  refreshBtn.className = "red-to-popup-refresh";
  ...
  list.className = "red-to-popup-list";
  ...
  hint.className = "red-to-popup-hint";
  ...
  const buildList = (cat, query) => {
    ...
    const addRow = (id, label, preview) => {
      ...
      item.className = "red-to-popup-item" + (id === currentId ? " active" : "");
    ...
    if (pinned) {
      ...
      sep.className = "red-to-popup-sep";
    ...
    if (shown === 0) {
      ...
      empty.className = "red-to-popup-empty";
...
export function injectFontPickerCSS() {
  if (_cssInjected || document.getElementById("red-fontpicker-css")) { _cssInjected = true; return; }
  _cssInjected = true;
  const s = document.createElement("style");
  s.id = "red-fontpicker-css";
  s.textContent = `
    ...
    .red-to-popup-mag { color: #888; font-size: 14px; }
    .red-to-popup-search input {
      flex: 1;
      min-width: 0;
    ...
    .red-to-popup-search input::placeholder { color: #777; }
    .red-to-popup-refresh {
    ...
    .red-to-popup-refresh:hover { border-color: var(--red-acc,#f66744); color: #fff; }
    .red-to-popup-refresh:disabled { opacity: 0.5; cursor: default; }
    .red-to-popup-list { overflow-y: auto; flex: 1; }
    .red-to-popup-empty { padding: 10px 12px; color: #777; font: 12px ui-sans-serif, system-ui, sans-serif; }
    .red-to-popup-item {
      padding: 6px 10px;
      cursor: pointer;
      border-bottom: 1px solid #2a2a2a;
    }
    .red-to-popup-item:last-child { border-bottom: none; }
    .red-to-popup-item:hover { background: #2a2a2a; }
    .red-to-popup-item.active { color: var(--red-acc,#f66744); font-weight: 600; }
    .red-to-popup-sep { height: 1px; background: #333; margin: 4px 0; }
