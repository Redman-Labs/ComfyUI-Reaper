// Moved from js/load_image/resize_modes.mjs so Image Resize Reaper can reuse
...
import { openReaperColorPickerModal } from "./color_picker.mjs";
import { BRAND as DEFAULT_BRAND } from "./utils.mjs";

// Resolve against the owning node first so shared resize controls follow that
// node's Reaper Default Style accent instead of always using the fallback brand.
const BRAND = `var(--red-acc, ${DEFAULT_BRAND})`;

// The per-mode panels use `rl-li-*` class names. Load Image injects these in
...
export function injectResizePanelCSS() {
  if (_resizePanelCSSInjected || document.getElementById("rl-resize-panel-css")) return;
  ...
  const css = `
    ...
    .rl-li-panel input[type="range"] { flex: 1; accent-color: ${BRAND}; }
    ...
    .rl-li-quickpicks { display: grid; gap: 3px; margin-bottom: 8px; }
    .rl-li-quickpick {
      background: #1d1d1d;
      border: 1px solid #444;
      border-radius: 3px;
    ...
    .rl-li-quickpick.active { background: ${BRAND}; color: #fff; border-color: ${BRAND}; }
    .rl-li-ratio-chips {
      display: grid;
    ...
    .rl-li-ratio-chip.active { background: ${BRAND}; color: #fff; border-color: ${BRAND}; }
    .rl-li-cropped {
      display: grid;
    ...
    .rl-li-cropped > div.active { background: ${BRAND}; color: #fff; }
    .rl-li-pad-row { display: flex; align-items: center; gap: 6px; font-size: 10px; color: #888; }
    .rl-li-pad-swatch {
    ...
    .rl-li-custom-ratio-row .rl-li-custom-ratio-swap {
      width: 28px;
      height: auto;
      align-self: stretch;
      min-height: 22px;
    ...
    .rl-li-custom-ratio-swap:hover { color: ${BRAND}; border-color: ${BRAND}; }
    .rl-li-panel-row.rl-li-centered { justify-content: center; }
    .rl-li-input-wide { width: 70% !important; max-width: 200px; }
    .rl-li-numinput {
      display: inline-flex;
      align-items: stretch;
      background: #2a2a2a;
    ...
    .rl-li-numinput:focus-within { border-color: ${BRAND}; }
    ...
    .rl-li-spin > button:hover { background: #333; color: ${BRAND}; }
    ...
    .rl-li-spin-up::before { transform: translate(-50%, -25%) rotate(-45deg); }
    .rl-li-spin-down::before { transform: translate(-50%, -75%) rotate(135deg); }
    .rl-li-wh-row {
      display: grid;
    ...
    .rl-li-wh-field { display: flex; flex-direction: column; gap: 3px; }
    .rl-li-wh-label {
      font-size: 9px;
      color: #888;
      text-transform: uppercase;
    ...
    .rl-li-swap:hover { color: ${BRAND}; border-color: ${BRAND}; }
    .rl-li-wh-preview {
      display: flex;
      flex-direction: column;
      align-items: center;
    ...
    .rl-li-wh-rect-label { font-size: 9px; color: #999; font-family: inherit; }
    ...
    .rl-li-ratio-chip.rl-li-ratio-custom-chip { width: 100%; }
    ...
    .rl-li-pad-top { grid-column: 2; grid-row: 1; }
    .rl-li-pad-left { grid-column: 1; grid-row: 2; }
    .rl-li-pad-right { grid-column: 3; grid-row: 2; }
    .rl-li-pad-bottom { grid-column: 2; grid-row: 3; }
    .rl-li-pad-mid {
    ...
    .rl-li-pad-input-wrap { width: 100%; max-width: 82px; }
    .rl-li-pad-inlabel {
    ...
    .rl-li-pad-labeled input { text-align: right !important; padding-right: 6px !important; }
    .rl-li-pad-outdims { font-size: 11px; font-weight: 600; color: ${BRAND}; }
    .rl-li-pad-outhint { font-size: 8px; color: #777; text-transform: uppercase; letter-spacing: 0.5px; line-height: 1.3; }
    .rl-li-pad-resetcell { grid-column: 1; grid-row: 3; display: flex; align-items: center; justify-content: center; }
    .rl-li-pad-reset {
    ...
    .rl-li-pad-reset:hover { border-color: ${BRAND}; color: ${BRAND}; }
    .rl-li-pad-reset-ic {
      width: 11px; height: 11px; flex: none; background-color: currentColor;
      -webkit-mask: url("/reaper/assets/icons/ui/reset.svg") center/11px 11px no-repeat;
              mask: url("/reaper/assets/icons/ui/reset.svg") center/11px 11px no-repeat;
    }
    .rl-li-pad-colorcell {
    ...
    .rl-li-pad-colorcell:hover { border-color: ${BRAND}; }
    .rl-li-pad-colorcell .rl-li-pad-swatch { width: 16px; height: 16px; }
    .rl-li-pad-colorlbl { font-size: 9px; color: #999; text-transform: uppercase; letter-spacing: 0.5px; }
  `;
  const s = document.createElement("style");
  s.id = "rl-resize-panel-css";
  s.textContent = css;
...
// Recursive-descent math evaluator copied from Resolution Reaper. Supports
...
function makePanelHeader(text) {
  ...
  lbl.className = "rl-li-panel-label";
...
function makeReadout(text) {
  ...
  ro.className = "rl-li-panel-readout";
  ro.textContent = text;
  return ro;
}
...
function buildMaxMPPanel(node, state, writeState, onChange, stateKey, extra = {}) {
  ...
  panel.className = "rl-li-panel";
  panel.appendChild(makePanelHeader("Max Megapixels"));

  const quickWrap = document.createElement("div");
  quickWrap.className = "rl-li-quickpicks";
  ...
  for (const v of QUICK) {
    ...
    q.className = "rl-li-quickpick" + (Math.abs(v - cur) < 0.001 ? " active" : "");
  ...
  row.className = "rl-li-panel-row rl-li-centered";
...
function buildLongestSidePanel(node, state, writeState, onChange, stateKey, extra = {}) {
  ...
  for (const v of QUICK) {
    const q = document.createElement("div");
    q.className = "rl-li-quickpick" + (v === cur ? " active" : "");
...
function buildWHPanel(node, state, writeState, onChange, opts, stateKey) {
  ...
  const row = document.createElement("div");
  row.className = "rl-li-wh-row";

  function makeField(labelText, value, key) {
    const wrap = document.createElement("div");
    wrap.className = "rl-li-wh-field";
    const lbl = document.createElement("div");
    lbl.className = "rl-li-wh-label";
    ...
    inpWrap.classList.add("rl-li-wh-input-wrap");
    inp.classList.add("rl-li-wh-input");
  ...
  previewWrap.className = "rl-li-wh-preview";
  let previewRect, innerRect = null;
  if (nested) {
    previewRect = document.createElement("div");
    previewRect.className = "rl-li-wh-frame";
    innerRect = document.createElement("div");
    innerRect.className = "rl-li-wh-out";
    previewRect.appendChild(innerRect);
  } else {
    previewRect = document.createElement("div");
    previewRect.className = "rl-li-wh-rect";
  }
  const previewLabel = document.createElement("div");
  previewLabel.className = "rl-li-wh-rect-label";
...
// 9 presets to match Resolution Reaper's chip grid + Custom in slot 10.
...
function buildMatchRatioPanel(node, state, writeState, onChange, stateKey, extra = {}) {
  ...
  chipsWrap.className = "rl-li-ratio-chips";
  const chipEls = [];
  for (const r of RATIO_PRESETS) {
    const el = document.createElement("div");
    el.className = "rl-li-ratio-chip" + (state.ratio_preset === r.id ? " active" : "");
    el.dataset.rid = r.id;
    if (r.id === "custom") {
      el.classList.add("rl-li-ratio-custom-chip");
  ...
  customRow.className = "rl-li-custom-ratio-row";
  ...
  cwBuilt.wrap.classList.add("rl-li-custom-ratio-input-wrap");
  const swapRatio = makeSwapButton("Swap ratio W ↔ H");
  swapRatio.classList.add("rl-li-custom-ratio-swap");
  ...
  chBuilt.wrap.classList.add("rl-li-custom-ratio-input-wrap");
  ...
  padRow.className = "rl-li-pad-row";
  padRow.innerHTML = `<span>Pad color</span>`;
  const swatch = document.createElement("div");
  swatch.className = "rl-li-pad-swatch";
  ...
  chipsWrap.addEventListener("click", (e) => {
    const el = e.target.closest(".rl-li-ratio-chip");
    if (!el) return;
    e.stopPropagation();
  ...
  // Photoshop-style modal picker - same one Text Overlay Reaper uses
...
function buildPadPanel(node, state, writeState, onChange, stateKey, extra = {}) {
  ...
  grid.className = "rl-li-padgrid";

  const center = document.createElement("div");
  center.className = "rl-li-pad-mid";
  const inDims = extra.inputDims || null;
  function updateCenter() {
    if (inDims && inDims.w && inDims.h) {
      const s = { ...state, ...JSON.parse(node.properties?.[stateKey] || "{}") };
      const { w, h } = previewResize(inDims.w, inDims.h, { ...s, mode: "pad" });
      center.innerHTML = `<div class="rl-li-pad-outdims">${w} × ${h}</div>`;
    } else {
      center.innerHTML = `<div class="rl-li-pad-outhint">output<br>size</div>`;
    }
  }
  ...
  function makePadField(letter, title, key, placeClass) {
    ...
    wrap.classList.add("rl-li-pad-input-wrap", "rl-li-pad-labeled", placeClass);
    wrap.title = title;
    const lab = document.createElement("span");
    lab.className = "rl-li-pad-inlabel";
  ...
  const topW = makePadField("T", "Top padding (px)", "pad_top", "rl-li-pad-top");
  const leftW = makePadField("L", "Left padding (px)", "pad_left", "rl-li-pad-left");
  const rightW = makePadField("R", "Right padding (px)", "pad_right", "rl-li-pad-right");
  const botW = makePadField("B", "Bottom padding (px)", "pad_bottom", "rl-li-pad-bottom");
