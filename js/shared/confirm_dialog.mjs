...
// reports in this pack. Never gate a Reaper action on a native dialog.
//
// This is the PROVEN shape already copy-pasted into find_replace / prompt_multi
// / prompt_stack as a local `redConfirm`, lifted here so a fifth consumer does
...
const CSS_ID = "reaper-confirm-dialog-css";
...
function injectCSS() {
  ...
  // 11000 for .rdx-overlay), so a confirm raised from INSIDE an editor is
  // visible and clickable over it.
  style.textContent = `
    .red-cfm-back { position:fixed; inset:0; z-index:12000;
      background:rgba(0,0,0,.62); display:flex; align-items:center;
      justify-content:center; }
    .red-cfm { width:min(430px,92vw); background:#232325; border:1px solid #3a3a3c;
      border-radius:8px; color:#e0e0e0; font:12px 'Segoe UI', sans-serif;
      box-shadow:0 12px 34px rgba(0,0,0,.55); overflow:hidden; }
    .red-cfm-title { padding:11px 14px; background:#2a2a2c;
      border-bottom:1px solid #1c1c1e; font-size:13px; font-weight:600; }
    .red-cfm-msg { padding:13px 14px 2px; font-size:12px; line-height:1.5;
      color:#b9b9b9; white-space:pre-wrap; }
    .red-cfm-actions { display:flex; gap:8px; justify-content:flex-end;
      padding:14px; }
    .red-cfm-btn { background:rgba(255,255,255,.05);
    ...
    .red-cfm-btn:hover { background:${BRAND}; border-color:${BRAND}; color:#fff; }
    .red-cfm-btn.primary { background:${BRAND}; border-color:${BRAND}; color:#fff; }
    .red-cfm-btn:focus-visible { outline:2px solid ${BRAND}; outline-offset:2px; }
  `;
 ...
 * so a caller can always write `if (!await redConfirm(...)) return;`.
 *
 ...
 */
export function redConfirm({ title, message, okText = "OK", cancelText = "Cancel",
                             danger = false } = {}) {
  injectCSS();
  return new Promise((resolve) => {
    const back = document.createElement("div");
    back.className = "red-cfm-back";
    const box = document.createElement("div");
    box.className = "red-cfm";

    const titleEl = document.createElement("div");
    titleEl.className = "red-cfm-title";
    titleEl.textContent = title || "Confirm";
    box.appendChild(titleEl);

    if (message) {
      const msgEl = document.createElement("div");
      msgEl.className = "red-cfm-msg";
      msgEl.textContent = message;      // textContent: a caller's text is never HTML
      box.appendChild(msgEl);
    }

    const actions = document.createElement("div");
    actions.className = "red-cfm-actions";
    const cancelBtn = document.createElement("button");
    cancelBtn.type = "button";
    cancelBtn.className = "red-cfm-btn";
    cancelBtn.textContent = cancelText;
    const okBtn = document.createElement("button");
    okBtn.type = "button";
    okBtn.className = "red-cfm-btn primary";
