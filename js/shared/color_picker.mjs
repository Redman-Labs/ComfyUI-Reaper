// Reaper Color Picker — reusable inline HSV + swatch + hex picker.
//
// Exposes:
//   - createReaperColorPicker(opts) → { element, getColor, setColor, destroy }
//       Inline component you append into any container.
//   - openReaperColorPickerPopup(anchorEl, opts)
//       Standalone popup (drop-in replacement for the old openColorPop).
//   - REAPER_PALETTE — the 36-color default swatch grid.
//
...
//
// "Reaper Color Picker" — see `feedback_reaper_color_picker.md`
// in the user's memory dir.

import { BRAND } from "./utils.mjs";

// ── Default 36-color palette ───────────────────────────────────────
export const REAPER_PALETTE = [
...
// ── Button-safe palette ────────────────────────────────────────────
// For accent colors that fill BUTTONS / pills with WHITE text on top
// (Sliders, Sizes, and any future orange-button node). Every color here
...
// The Reaper brand orange leads; the rest are Tailwind-600-ish shades
// (designed for white text) spanning the hue wheel + a few neutrals.
export const BUTTON_PALETTE = [
  "#f66744", // Reaper orange (brand)
  "#dc2626", // red
  "#ea580c", // orange
  "#d97706", // amber
  "#ca8a04", // gold
...
.rl-cp-popup.rl-cp-popup-wide { width: 300px; padding: 12px; }
.rl-cp-popup-wide .rl-cp-swatches { gap: 4px; }
.rl-cp-popup-wide .rl-cp-sv { height: 190px; }
.rl-cp-popup-wide .rl-cp-hex,
.rl-cp-popup-wide .rl-cp-reset { height: 28px; font-size: 12px; }
...
.rl-cp-popup.rl-cp-popup-compact {
  width: 320px;
  padding: 12px;
}
.rl-cp-popup.rl-cp-popup-compact .rl-cp-swatches {
  gap: 4px;
}
.rl-cp-compact-footer {
  display: flex;
  gap: 8px;
  margin-top: 10px;
}
.rl-cp-compact-footer .rl-cp-reset {
  flex: 0 0 auto;
  height: 28px;
  padding: 0 14px;
  font-size: 12px;
...
.rl-cp-modal-btn:hover {
  color: ${BRAND};
  border-color: ${BRAND};
}
.rl-cp-modal-btn.primary {
  background: ${BRAND};
  border-color: ${BRAND};
  color: #fff;
}
.rl-cp-modal-btn.primary:hover {
  background: #d54f2c;
  border-color: #d54f2c;
  color: #fff;
}
...
//   swatches: array of hex strings (default REAPER_PALETTE)
...
export function createReaperColorPicker(opts = {}) {
  ensureCSS();
  const {
    initialColor = "#f66744",
  ...
  root.className = "rl-cp";

  // ── Swatches (with optional Clear / transparent tile) ───────────
  const swatchGrid = document.createElement("div");
  swatchGrid.className = "rl-cp-swatches";
  ...
  if (showClear) {
    ...
    clearTile.className = "rl-cp-tile rl-cp-tile-clear";
  ...
  svRow.className = "rl-cp-sv-row";

  const svCanvas = document.createElement("canvas");
  svCanvas.width  = 168;
  svCanvas.height = 80;
  svCanvas.className = "rl-cp-sv";

  const hueCanvas = document.createElement("canvas");
  hueCanvas.width  = 14;
  hueCanvas.height = 80;
  hueCanvas.className = "rl-cp-hue";
  ...
  hexRow.className = "rl-cp-hexrow";

  const hexInput = document.createElement("input");
  hexInput.type = "text";
  hexInput.className = "rl-cp-hex";
  ...
  if (!hideReset) {
    ...
    resetBtn.className = "rl-cp-reset";
...
//   swatches:     array (default REAPER_PALETTE)
//   showClear:    bool
//   resetColor:   hex (default "#f66744")
//   onPick:       (color | null) => void
export function openReaperColorPickerPopup(anchorEl, opts = {}) {
  ensureCSS();
  const popup = document.createElement("div");
  popup.className = "rl-cp-popup";
  if (opts.wide) popup.classList.add("rl-cp-popup-wide");
  const rect = anchorEl.getBoundingClientRect();
  popup.style.left = `${rect.left}px`;
  popup.style.top  = `${rect.bottom + 4}px`;

  const picker = createReaperColorPicker({
...
//   onPick:       (color | null) => void
export function openReaperCompactColorPickerPopup(anchorEl, opts = {}) {
  ensureCSS();

  const swatches      = opts.swatches    ?? REAPER_PALETTE;
  const showClear     = !!opts.showClear;
  // clearPosition: "first" (default, legacy) or "last". Last is what
  // every Reaper Note picker (text / highlight / Bg) currently uses
  ...
  const popup = document.createElement("div");
  popup.className = "rl-cp-popup rl-cp-popup-compact";
  const rect = anchorEl.getBoundingClientRect();
  popup.style.left = `${rect.left}px`;
  popup.style.top  = `${rect.bottom + 4}px`;
  ...
  const buildClearTile = () => {
    const tile = document.createElement("button");
    tile.type = "button";
    tile.className = "rl-cp-tile rl-cp-tile-clear";
  ...
  footer.className = "rl-cp-compact-footer";
  ...
  moreBtn.type = "button";
  moreBtn.className = "rl-cp-more-btn";
  ...
  moreBtn.addEventListener("click", (e) => {
    ...
    openMoreColorsModal({
      ...
      // fine-tuning starts from their current color, not BRAND).
...
export function openReaperColorPickerModal(opts = {}) {
  ...
  backdrop.className = "rl-cp-modal-backdrop";

  const box = document.createElement("div");
  box.className = "rl-cp-modal-box";

  const titleEl = document.createElement("div");
  titleEl.className = "rl-cp-modal-title";
  ...
  actions.className = "rl-cp-modal-actions";

  const cancelBtn = document.createElement("button");
  cancelBtn.type = "button";
  cancelBtn.className = "rl-cp-modal-btn";
  cancelBtn.textContent = "Cancel";

  const applyBtn = document.createElement("button");
  applyBtn.type = "button";
  applyBtn.className = "rl-cp-modal-btn primary";
...
// when `.rl-cp-modal-backdrop` is in the DOM, see Pattern #27).
function openMoreColorsModal(opts) {
  ...
  title.className = "rl-cp-modal-title";
  ...
  okBtn.type = "button";
  okBtn.className = "rl-cp-modal-btn primary";
  ...
  // for an open `.rl-cp-modal-backdrop` and returns early, so this
