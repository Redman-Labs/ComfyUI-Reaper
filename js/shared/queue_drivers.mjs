// Shared guard for Reaper "queue-driver" nodes (Prompt Multi, Prompt Pack).
//
...
//    holds the lock; any nested Reaper queue wrapper sees the lock and falls
//    through to a single pass-through call instead of looping again.
//
// 2. INACTIVE SWITCH BRANCH. A Switch Reaper routes only ONE of its inputs
...
const SWITCH_CLASS = "ReaperSwitch";
const SWITCH_STATE_PROP = "switchState";
const SWITCH_SOURCE_CLASS = "ReaperSwitchSource";
...
// True while a Reaper queue-driver is mid-loop. A nested driver wrapper
...
// on an inactive input of a Switch Reaper. Such a node cannot reach any
