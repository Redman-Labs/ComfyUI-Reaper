// Any Switch Advanced (Reaper) - slot management and state helpers.
//
...
export function normalizeSlots(node) {
  ...
  node._reaperAdvSwRefresh?.(); // re-render the Nodes 2.0 DOM list (no-op in legacy)
}
...
// (vue_list.mjs). node._reaperAdvSwRefresh re-renders the Vue list when present
...
export function handleConnect(node, slotIdx1) {
  ...
  // node._reaperAdvSwRestoring is raised in the `configure` WRAPPER (index.js - NOT
  ...
  if (!node._reaperAdvSwRestoring) state.activeIndex = slotIdx1;
