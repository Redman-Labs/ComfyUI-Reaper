// Any Switch Advanced (Reaper) - Nodes 2.0 rows, each with its input dot ON THE ROW.
//
...
const ROW_WIDGET_NAME = (idx1) => `reaper_adv_sw_row_${idx1}`;

function injectCSS() {
  if (document.getElementById("reaper-adv-sw-css")) return;
  const s = document.createElement("style");
  s.id = "reaper-adv-sw-css";
  s.textContent = `
    .reaper-adv-sw-row {
    ...
    .reaper-adv-sw-row:hover { border-color:rgba(255,255,255,0.30); }
    .reaper-adv-sw-row.active { border-color:var(--red-acc,#f66744); background:color-mix(in srgb, var(--red-acc,#f66744) 12%, transparent); }
    .reaper-adv-sw-row.active:hover { border-color:var(--red-acc,#f66744); }
    .reaper-adv-sw-row.trailing { cursor:default; opacity:0.5; border-style:dashed; }
    .reaper-adv-sw-row.trailing:hover { border-color:rgba(255,255,255,0.12); }
    .reaper-adv-sw-num { flex:none; min-width:50px; color:rgba(255,255,255,0.55); font:12px 'Segoe UI',-apple-system,sans-serif; white-space:nowrap; }
    .reaper-adv-sw-label {
      flex:1; min-width:0; color:#d8d8d8; font:12px 'Segoe UI',-apple-system,sans-serif;
      white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
    }
    .reaper-adv-sw-row.trailing .reaper-adv-sw-label { color:#aaa; font-style:italic; }
    ...
    .reaper-adv-sw-name:hover { border-color:rgba(255,255,255,0.18); }
    .reaper-adv-sw-name:focus { border-color:var(--red-acc,#f66744); background:#1d1d1d; }
    .reaper-adv-sw-name::placeholder { color:#888; }
    .reaper-adv-sw-toggle {
      position:relative; width:28px; height:14px; border-radius:7px; flex:none; box-sizing:border-box;
      border:1px solid rgba(255,255,255,0.18); background:rgba(255,255,255,0.06);
    }
    .reaper-adv-sw-row.active .reaper-adv-sw-toggle { border-color:var(--red-acc,#f66744); background:var(--red-acc,#f66744); }
    .reaper-adv-sw-knob { position:absolute; top:2px; left:2px; width:8px; height:8px; border-radius:50%; background:#ccc; transition:left .1s ease; }
    .reaper-adv-sw-row.active .reaper-adv-sw-knob { left:16px; background:#fff; }
    .reaper-adv-sw-row.trailing .reaper-adv-sw-toggle { visibility:hidden; }
    ...
    .lg-node-widget:has(.reaper-adv-sw-row) > div:first-child { opacity:1 !important; }
  `;
...
function syncRowWidgets(node) {
  ...
  const rows = node._reaperAdvSwRows || (node._reaperAdvSwRows = []);
  ...
  while (rows.length < inputs.length) {
    ...
    el.className = "reaper-adv-sw-row";
...
function renderRows(node) {
  const inputs = node.inputs || [];
  const rows = node._reaperAdvSwRows || [];
  ...
  for (let i = 0; i < rows.length && i < inputs.length; i++) {
    ...
    rowEl.className =
      "reaper-adv-sw-row" + (isActive ? " active" : "") + (isTrailing ? " trailing" : "");
    rowEl.innerHTML = "";

    // Left: fixed "input N" identity (matches the slot name Python routes on).
    const numEl = document.createElement("span");
    numEl.className = "reaper-adv-sw-num";
    ...
    if (connected && !isTrailing) {
      ...
      labelEl.type = "text";
      labelEl.className = "reaper-adv-sw-name";
      ...
      labelEl.className = "reaper-adv-sw-label";
    ...
    toggleEl.className = "reaper-adv-sw-toggle";
    const knob = document.createElement("span");
    knob.className = "reaper-adv-sw-knob";
...
export function buildSwitchVueList(node) {
  ...
  node._reaperAdvSwRefresh = refresh;
  refresh();
...
export function teardownSwitchVueList(node) {
  ...
  node._reaperAdvSwRows = [];
  node._reaperAdvSwRefresh = null;
