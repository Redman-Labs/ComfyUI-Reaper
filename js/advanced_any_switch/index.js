...
// Any Switch Advanced (Reaper) - dynamic N-to-1 switch with per-row toggles.
// Rendering follows the Image Compare Reaper pattern: onDrawForeground
...
// True while a workflow is loading. The per-node _reaperAdvSwitchConfiguring flag
...
if (app && app.loadGraphData && !app._reaperAdvSwLoadWrapped) {
  app._reaperAdvSwLoadWrapped = true;
...
function applyRenderer(node, vue) {
  try {
    if (vue) {
      ...
      node._reaperAdvSwLegacyH = node.size?.[1];
      ...
      const h = Math.max(min, node._reaperAdvSwLegacyH || 0);
    ...
    console.warn("[Reaper] Switch renderer rebuild failed", err);
  }
}

app.registerExtension({
  name: "Reaper.AdvancedAnySwitch",

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "ReaperAdvancedAnySwitch") return;

    // ── Creation ─────────────────────────────────────────────────────────
    const _origCreated = nodeType.prototype.onNodeCreated;
    nodeType.prototype.onNodeCreated = function () {
      _origCreated?.apply(this, arguments);
      hideJsonWidget(this.widgets, HIDDEN_INPUT_NAME);
      setupNode(this);
      // Nodes 2.0 only: build the DOM row list (one row per input). It wires
      // itself as node._reaperAdvSwRefresh, which core.mjs calls on every slot/state
      ...
      this._reaperAdvSwRendererOff = onRendererChange((vue) => applyRenderer(this, vue));
    ...
    nodeType.prototype.onRemoved = function () {
      ...
      this._reaperAdvSwRendererOff?.();
      this._reaperAdvSwRendererOff = null;
      if (this._reaperAdvSwRestoreTimer) {
        clearTimeout(this._reaperAdvSwRestoreTimer);
        this._reaperAdvSwRestoreTimer = null;
        this._reaperAdvSwRestoring = false;
      }
    ...
    // cap. So the long-standing `_reaperAdvSwitchConfiguring` flag (Vue Compat #17)
    ...
    nodeType.prototype.configure = function () {
      this._reaperAdvSwitchConfiguring = true;
      try {
        ...
        if (typeof _origConfigureFn !== "function") {
          console.error("[Any Switch Advanced (Reaper)] node configure() is missing - saved state was not restored");
          return undefined;
        }
        return _origConfigureFn.apply(this, arguments);
      } finally {
        this._reaperAdvSwitchConfiguring = false;
        ...
        this._reaperAdvSwRestoring = true;
        clearTimeout(this._reaperAdvSwRestoreTimer);
        this._reaperAdvSwRestoreTimer = setTimeout(() => {
          this._reaperAdvSwRestoring = false;
          this._reaperAdvSwRestoreTimer = null;
        }, 0);
      }
    ...
    nodeType.prototype.onConfigure = function (info) {
      ...
      // _reaperAdvSwitchConfiguring up for the whole of configure, INCLUDING this
    ...
    // ── Connection changes ────────────────────────────────────────────────
    // Skip handleConnect / handleDisconnect while _reaperAdvSwitchConfiguring is
    ...
    // defense from before _reaperAdvSwitchConfiguring existed. It now actively
    ...
    nodeType.prototype.onConnectionsChange = function (
      ...
      // Two guards: _reaperAdvSwitchConfiguring (this node's onConfigure window) and
      ...
      if (type === 1 /* INPUT */ && !this._reaperAdvSwitchConfiguring && !_swLoadingGraph) {
...
// submission time. Pattern #9 (same as Resolution Reaper / Switch WH).
...
function buildSwitchNodeIndex() {
  ...
  const visit = (graph) => {
    ...
    for (const n of nodes) {
      if (!n) continue;
      if (n.comfyClass === "ReaperAdvancedAnySwitch" || n.type === "ReaperAdvancedAnySwitch") {
...
app.graphToPrompt = async function (...args) {
  ...
  try {
    ...
    if (out) {
      ...
      for (const id in out) {
        const entry = out[id];
        if (!entry || entry.class_type !== "ReaperAdvancedAnySwitch") continue;
    ...
    console.error("[Reaper] Switch prompt injection failed; prompt sent unchanged", e);
  }
  return result;
};
...
if (!api._reaperAdvSwQueueWrapped) {
  api._reaperAdvSwQueueWrapped = true;
  ...
  api.queuePrompt = async function (...args) {
    try {
      ...
      console.warn("[Any Switch Advanced (Reaper)] could not prune inactive inputs:", (e && e.message) || e);
    }
...
registerNodeAccent("ReaperAdvancedAnySwitch", { title: "Any Switch Advanced" });
