// Pure paint helpers for Any Switch Advanced (Reaper) rows.
...
// Mirrors the Note Reaper pencil-position math.
...
function drawToggle(ctx, nodeWidth, slotIdx0, on, disabled) {
  ...
  // -> Colors (matches Text Reaper and Switch WH adaptive style). ON
  // stays opaque BRAND orange so the active toggle is unambiguous on any
  // node colour. Slightly higher opacity than Text Reaper's action
