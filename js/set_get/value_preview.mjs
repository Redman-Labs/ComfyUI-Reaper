// ╔═══════════════════════════════════════════════════════════════╗
// ║  SetNode (Reaper) / GetNode (Reaper) - best-effort value readout               ║
// ╚═══════════════════════════════════════════════════════════════╝
//
...
import { installCanvasZoomPassthrough } from "../shared/index.mjs";
...
// Round half to even, matching Python's round() (and Number Reaper's int
...
function fmt(v, type) {
  ...
  if (typeof v === "number") {
    ...
    // a decimal (e.g. Number Reaper 5.5 -> its int output is 6), so round.
...
export function ensureValueWidget(node) {
  ...
  if (node._redSgValEl) return node._redSgValEl;
  try {
    injectCSS();
    const el = document.createElement("div");
    el.className = "red-sg-val";
    el.style.display = "none";
    installCanvasZoomPassthrough(el);
    const w = node.addDOMWidget("red_setget_value", "reaper_setget_value", el, {
      serialize: false,
      getMinHeight: () => (node._redSgValShown ? ROW_H : 0),
      getMaxHeight: () => (node._redSgValShown ? ROW_H : 0),
    });
    w.computeSize = () => [node.size?.[0] || 140, node._redSgValShown ? ROW_H : 0];
    applyAdaptiveCanvasOnly(w);
    node._redSgValEl = el;
    node._redSgValWidget = w;
    return el;
  } catch {
    return null;
  }
...
export function paintReadout(node, ctx) {
  ...
  if (!node._redSgValShown || !node._redSgValText || !ctx) return;
  ...
  ctx.fillText("= " + node._redSgValText, 12, anchor + 13);
  ctx.restore();
}
...
function fitNodeForReadout(node) {
  if (isVue() || node._redSgValShown) return;
  try {
...
export function refreshValue(node) {
  ...
  const prevShown = !!node._redSgValShown;
  const prevText = node._redSgValText || "";
  node._redSgValShown = show;
  node._redSgValText = show ? val : "";

  // Nodes 2.0: update the DOM element (created only in Vue mode).
  const el = node._redSgValEl;
  if (el) {
...
export function startValuePoll() {
  ...
  if (window.__redSgValPoll) return;
  window.__redSgValPoll = setInterval(() => {
    const g = app.canvas?.graph || app.graph;
    if (!g?._nodes) return;
    for (const n of g._nodes) {
      // A Get mirrors its Set's color - keep it synced even when collapsed.
      if (n.type === GET_TYPE) {
        ...
        try {
          const want = n.properties?.redSGName;
          if (
            want &&
            !n.widgets?.[0]?.value &&
            !isGraphLoading() &&
