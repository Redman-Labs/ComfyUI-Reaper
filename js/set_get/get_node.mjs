// ╔═══════════════════════════════════════════════════════════════╗
// ║  GetNode (Reaper) - virtual "named variable" source              ║
// ╚═══════════════════════════════════════════════════════════════╝
//
// Minimal virtual node: a dropdown that picks which SetNode (Reaper) to read (by
...
//   - resolveVirtualOutput(slot) → cross-graph / subgraph (native 1.47.10+ path)
...
export function registerReaperGetNode() {
  const LGraphNode = LiteGraph.LGraphNode;

  class ReaperGetNode extends LGraphNode {
    static title = "GetNode (Reaper)";
    ...
    constructor(title) {
      ...
      // LiteGraph assigns a generic color in super(), so a truthiness guard
      // would prevent this category default from ever being applied. Saved
      // workflow colors are restored later by configure().
      ...
      widget.onClick = (params) => {
        ...
        // Color each entry's left edge by that Set's own color.
    ...
    setName(name) {
      ...
      if (this.properties.redSGName !== name) this.properties.redSGName = name;
      this.onRename();
    }
    ...
    onRename() {
      ...
      if (name && this.properties.redSGName !== name) this.properties.redSGName = name;
      if (setter) {
        this.setType(firstWiredInput(setter)?.type ?? "*");
        this.title = `GetNode: ${name}`;
      } else {
        this.setType("*");
        this.title = name ? `GetNode: ${name}` : "GetNode (Reaper)";
      }
      inheritSetColor(this); // take the chosen Set's color
      refreshValue(this);
    ...
    // Subgraph-aware path (native in 1.47.10+). Returns the REAL source
    ...
    onConfigure() {
      ...
      const savedName = this.properties.redSGName;
      if (savedName && !this.widgets[0].value) {
        this.widgets[0].value = savedName;
      } else if (
        this.widgets[0].value &&
        this.properties.redSGName !== this.widgets[0].value
      ) {
        this.properties.redSGName = this.widgets[0].value;
      }
  ...
  LiteGraph.registerNodeType(GET_TYPE, ReaperGetNode);
}
