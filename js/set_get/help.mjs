// Help content for SetNode (Reaper) / GetNode (Reaper), shown by the orange ? button in the node
...
registerNodeHelp("ReaperSetNode", {
  title: "SetNode (Reaper)",
  tagline: "Store any wire under a name, then read it back anywhere with GetNode (Reaper).",
  sections: [
    {
      heading: "What it does",
      body:
        "SetNode (Reaper) is a wireless wire. Connect anything into it (an image, a model, a number, a prompt) and give it a name. A GetNode (Reaper) anywhere in the workflow can then read that same value by picking the name, with no cable running across the canvas.\n\nIt also has a passthrough output: wire it straight to a node sitting nearby, and use Get nodes for the ones far away - same value either way.\n\nIt lives only in the editor. At run time the value flows straight from the original source, exactly as if you had wired it directly, so it never slows anything down or changes the result.",
    },
    {
      heading: "How to use it",
      bullets: [
        "Drag a connection into the input. The node takes on that wire's type and automatically uses the connected output name.",
        "Duplicate names are numbered automatically (for example MODEL, MODEL_0, MODEL_1). You can edit the generated name whenever needed.",
        "Wire the output to a nearby node directly, or add GetNode (Reaper) nodes for far-apart ones.",
    ...
    },
    {
      heading: "Colors",
      body:
        "Color a Set however you like (right-click -> Colors). Any Get that reads it takes the same color, so matching pairs are easy to spot, and the Gets follow along when you recolor the Set later. Turn off 'Get matches its Set's color' in Settings (under Reaper) to leave Gets on their own color.",
    },
    {
      heading: "Right-click menu",
      defs: [
        ["Select all its Gets", "Highlights every Get node in this graph that reads this name."],
        ["Add paired GetNode (Reaper)", "Drops a Get already set to this name next to the node."],
...
registerNodeHelp("ReaperGetNode", {
  title: "GetNode (Reaper)",
  tagline: "Read a value that a SetNode (Reaper) stored under a name, with no cable.",
  sections: [
    {
      heading: "What it does",
      body:
        "GetNode (Reaper) is the other half of the wireless wire. Pick the name of a SetNode (Reaper) from its dropdown and the output carries that Set's value, matching its type and color. Wire the output wherever you need it.\n\nLike Set, it exists only in the editor. At run time the connection resolves straight to the original source, so there is no extra cost and the result is identical to a direct cable.",
    },
    {
      heading: "How to use it",
      bullets: [
    ...
    },
    {
      heading: "Colors",
      body:
        "A Get takes the color of the Set it reads, so a matching pair is easy to spot, and each name in the dropdown is tagged with that Set's color. Color the Set (right-click -> Colors) and the Get follows. Turn off 'Get matches its Set's color' in Settings (under Reaper) to leave Gets alone.",
    },
    {
      heading: "Right-click menu",
      defs: [
        ["Jump to its SetNode (Reaper)", "Selects and centres the Set this Get is reading, even across subgraphs."],
      ],
    },
    {
      heading: "Subgraphs (important)",
    ...
    },
    {
      ...
      defs: [
        ["name", "A list of every SetNode (Reaper) name in this workflow. Pick one to read that value out here."],
      ],
    },
  ],
  footer: "If the dropdown is empty, add a SetNode (Reaper) first and give it a name.",
});
