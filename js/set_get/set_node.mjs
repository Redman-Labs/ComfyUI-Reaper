// ╔═══════════════════════════════════════════════════════════════╗
// ║  SetNode (Reaper) - virtual "named variable" sink                 ║
// ╚═══════════════════════════════════════════════════════════════╝
//
...
// value back from anywhere with GetNode (Reaper) - same value either way. Purely
...
const CATEGORY_TITLE_COLOR = "#2c2c2c";
const CATEGORY = "Reaper/Tools";

export function registerReaperSetNode() {
  const LGraphNode = LiteGraph.LGraphNode;

  class ReaperSetNode extends LGraphNode {
    static title = "SetNode (Reaper)";
    ...
    constructor(title) {
      ...
      // This custom frontend class replaces ComfyUI's generated definition, so
      // the category-default extension cannot style it. LiteGraph has already
      // assigned its generic color in super(), therefore this must be
      // unconditional for a newly constructed node. Workflow configuration
      // runs afterward and can still restore a saved/custom color.
      this.color = CATEGORY_TITLE_COLOR;
    ...
    refreshTitle() {
      ...
      this.title = v ? `SetNode: ${v}` : "SetNode (Reaper)";
    }
    ...
    // Set the input AND the passthrough output to one type. Color is left to
    ...
    onConnectionsChange(slotType, slot, isConnect, link_info) {
      ...
      // Input connect → adopt the upstream type and, for a new unnamed Set,
      // use the connected output's displayed name as the variable name.
      if (link_info && this.graph && slotType === LiteGraph.INPUT && isConnect) {
        ...
        if (typeof link_info.resolve === "function") {
          ...
          resolvedSlot = r?.subgraphInput ?? r?.output;
          type = resolvedSlot?.type;
        }
        // Read the original output as well. Its label/name is the most useful
        // automatic variable name, and it is also a fallback for the type when
        // the link resolver is unavailable in a particular graph renderer.
        const sourceNode = this.graph.getNodeById(link_info.origin_id);
        const sourceSlot = sourceNode?.outputs?.[link_info.origin_slot];
        type ||= sourceSlot?.type;
        if (type) {
          const currentName = String(this.widgets?.[0]?.value ?? "").trim();
          if (!currentName || currentName === "*") {
            ...
            if (suggestedName && suggestedName !== "*") {
              this.widgets[0].value = suggestedName;
            }
          }
          this.setAdoptedType(type);
          // MODEL, MODEL_0, MODEL_1, ... within this graph's visible scope.
          this.validateName(this.graph);
          this.properties.previousName = this.widgets[0].value;
        }
      }
    ...
    getExtraMenuOptions(_, options) {
      ...
      options.unshift({
        content: "Add paired GetNode (Reaper)",
        callback: () => {
  ...
  LiteGraph.registerNodeType(SET_TYPE, ReaperSetNode);
}
