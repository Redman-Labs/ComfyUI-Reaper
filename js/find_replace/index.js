...
// Reaper's shared help registry owns its panel lifecycle globally.
...
app.registerExtension({
  name: "Reaper.FindReplace",

  beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "ReaperFindReplace") return;
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      queueMicrotask(() => {
        ...
        // The V3 schema keeps the serialized state as an advanced STRING so
        // graphToPrompt can populate it. The custom editor replaces that raw
        // widget completely, so keep it out of both legacy and Nodes 2.0 layout.
        const stateWidget = node.widgets?.find((w) => w.name === "FindReplaceState");
        if (stateWidget) {
          stateWidget.type = "hidden";
          stateWidget.hidden = true;
          stateWidget.computeSize = () => [0, -4];
          stateWidget.computeLayoutSize = () => ({ minHeight: 0, minWidth: 0 });
        }
    ...
    nodeType.prototype.onExecuted = function (message) {
      ...
      try {
        ...
        if (data && typeof data.input === "string") {
          ...
          console.error("Reaper.FindReplace: onExecuted failed", err);
      }
    };
...
function buildPixFrNodeIndex() {
  ...
  const visit = (graph, prefix) => {
    ...
    for (const n of nodes) {
      ...
      if (n.comfyClass === "ReaperFindReplace" || n.type === "ReaperFindReplace") {
        index.set(fullId, n);
      }
  ...
  try {
    ...
    if (prompt && typeof prompt === "object") {
      ...
      for (const key of Object.keys(prompt)) {
        ...
        if (!entry || entry.class_type !== "ReaperFindReplace") continue;
    ...
    console.error("Reaper.FindReplace: graphToPrompt hook failed", err);
  }
  return result;
};
...
registerNodeAccent("ReaperFindReplace", { title: "Find and Replace" });
