...
// This remains mutable only for the duration of the synchronous canvas paint.
...
registerNodeHelp("ReaperCompareAdvanced", COMPARE_HELP);
...
const SIZE_FONT = "11px 'Segoe UI',sans-serif"; // bigger size text (was 9px)
...
function drawSizeLabels(ctx, node, W) {
  ...
  const sizeCol = sizesDiffer(node) ? "#cfcfcf" : "#cfcfcf";
  if (has1) {
    ctx.save();
    ctx.beginPath();
  ...
  settings: [
    {
      ...
      category: ["Reaper", "Image Compare"],
    },
  ],
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "ReaperCompareAdvanced") return;
