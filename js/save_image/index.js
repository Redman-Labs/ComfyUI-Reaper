// Save Image Reaper — extension entry point.
...
import { redApiUrl } from "../shared/api_url.mjs?reaper=2";
import {
...
} from "../shared/index.mjs?reaper=2";
import { isGraphLoading } from "../shared/graph_loading.mjs?reaper=2";
import { registerNodeSettings, installNodeAccent } from "../shared/node_settings.mjs?reaper=2";
import { applyFilenameTokenRefs } from "../shared/filename_tokens.mjs?reaper=2";
import {
  COMFY_CLASS,
  HIDDEN_INPUT_NAME,
  DEFAULT_STATE,
  ...
  formatDef,
  visibleFormats,
} from "./state.mjs?reaper=2";
import { injectCSS, buildRoot, el } from "./ui.mjs?reaper=3";
import { openSettingsPanel, closeSettingsPanelFor } from "./settings.mjs?reaper=3";
...
const CHIPS = [
  { label: "+ Input name", tok: "%input%", title: "Insert the wired name input (e.g. the filename from Load Image Reaper)" },
  ...
  { label: "+ Seed", tok: "%Seed Reaper.seed%", title: "Insert the seed from a Seed Reaper node" },
...
function measureFloor(ui) {
  ...
  if (!root || !root.isConnected || root.clientWidth < MIN_W - 40) {
    return ui._redSiFloorCache || 320;
  }
  let h = 0;
  let n = 0;
  ...
  if (n === 0) return ui._redSiFloorCache || 320; // pre-attach placeholder
  h += 16; // inner vertical padding (8 + 8)
  h += (n - 1) * 10; // flex gaps
  const out = Math.min(Math.max(200, h), FLOOR_CAP);
  ui._redSiFloorCache = out;
  return out;
}
...
function reassertSavedSize(node, saved) {
  ...
  const mounted = () => {
    const r = node._redSiUI && node._redSiUI.root;
  ...
  function retire() {
    ...
    ro = null;
    if (node._redSiReassertOff === retire) node._redSiReassertOff = null;
  }

  // Correct if wrong; retire only once laid out AND already correct.
  const apply = (mayRetire) => {
    if (done || !node._redSiUI || node._redSiSkipReassert) return;
    const off =
  ...
  try { node._redSiReassertOff?.(); } catch {} // a previous configure must not linger
  node._redSiReassertOff = retire;

  window.addEventListener("pointerdown", onDown, true);
  document.addEventListener("visibilitychange", onVis);
  try {
    const root = node._redSiUI && node._redSiUI.root;
...
function growToFloor(node) {
  if (!node._redSiUI) return;
  requestAnimationFrame(() => {
    if (!node._redSiUI || isGraphLoading()) return;
...
async function pickNativeFolder(startPath) {
  try {
    const url = redApiUrl(`/reaper/api/save_image/pick_native?path=${encodeURIComponent(startPath || "")}`);
...
function updateInfoLine(node) {
  ...
  if (!ui) return;
  const parts = [];
  const d = node._redSiLastDims;
  if (d) parts.push(d.w + " × " + d.h);
  if (node._redSiSummary) parts.push(node._redSiSummary);
  ui.infoLine.textContent = parts.join("   ·   ");
  ui.infoLine.title = node._redSiFolderInfo || "";
...
function flashStatus(node, kind, text, ms = 2600) {
  ...
  clearTimeout(node._redSiFlashT);
  node._redSiFlashT = setTimeout(() => updateInfoLine(node), ms);
}
...
function entrySrc(f) {
  ...
  if (f && f.token) return redApiUrl("/reaper/api/save_image/file?t=" + encodeURIComponent(f.token));
  return null;
}
...
function renderPreviewUI(node) {
  const ui = node._redSiUI;
  if (!ui) return;
  const frames = node._redSiFrames || [];
  const n = frames.length;
  let sel = node._redSiSel || 0;
  if (sel >= n) sel = n - 1;
  if (sel < 0) sel = 0;
  node._redSiSel = sel;
  ...
  const gridmode = n > 1 && !node._redSiExpanded;
  ...
  if (gridmode) {
    ...
    const total = Math.max(node._redSiTotal || 0, n);
    frames.forEach((f, i) => {
      const cell = el("div", "red-si-cell");
      const im = el("img");
      im.loading = "lazy";
      im.src = f.src;
      im.onerror = () => {
      ...
      cell.appendChild(im);
      cell.appendChild(el("div", "red-si-cellbadge", (i + 1) + " / " + total));
      cell.addEventListener("click", () => {
        node._redSiSel = i;
        node._redSiExpanded = true;
...
function stepPreview(node, dir) {
  const n = (node._redSiFrames || []).length;
  if (n < 2) return;
  node._redSiSel = ((node._redSiSel || 0) + dir + n) % n;
  renderPreviewUI(node);
}
...
async function copyFrame(node) {
  const ui = node._redSiUI;
  const frames = node._redSiFrames || [];
  const f = frames[node._redSiSel || 0];
  if (!ui || !f) return;
  try {
...
function downloadFrame(node) {
  const f = (node._redSiFrames || [])[node._redSiSel || 0];
  if (!f) return;
...
function openImageMenu(node, x, y) {
  closeImageMenu();
  const menu = el("div", "red-si-menu");
  _imgMenu = menu;
  _imgMenuNode = node;
  const add = (label, fn) => {
    const it = el("div", "red-si-mitem", label);
...
function resolveWiredName(node) {
  try {
    ...
    if (origin.comfyClass === "ReaperLoadImage") {
    ...
    // a plain text-ish widget on the origin (Text Reaper etc.) — best effort
...
function scheduleCounterFetch(node, folderRaw, nameWithExt, digits) {
  ...
  if (node._redSiCntKey === key) return; // already fetched / in flight
  node._redSiCntKey = key;
  clearTimeout(node._redSiCntTimer);
  node._redSiCntTimer = setTimeout(async () => {
    try {
      const r = await fetch(
        redApiUrl(
          "/reaper/api/save_image/next_counter?folder=" + encodeURIComponent(folderRaw) +
          "&name=" + encodeURIComponent(nameWithExt) +
          "&digits=" + encodeURIComponent(digits)
        )
      );
      const j = await r.json();
      if (node._redSiCntKey !== key || !node._redSiUI) return; // superseded
      node._redSiCounterNum = (j && j.counter) || 1;
      ...
      node._redSiCntResolved = (j && j.resolved) || "";
      ...
      node._redSiCntDenied = !!(j && j.denied);
      node._redSiCntResolvedFor = key;
      updatePreview(node);
    } catch {}
  }, 350);
}
...
function updatePreview(node) {
  ...
  s = applyFilenameTokenRefs(s); // %Seed Reaper.seed% and friends
  const dims = node._redSiLastDims;
  if (dims) {
    s = s.replace(/%width%/g, String(dims.w)).replace(/%height%/g, String(dims.h));
  }
  s = s.replace(/%batch_num%/g, "0");
  // Display mirror of the Python sanitizer, shared with Save Video Reaper -
  ...
  let rel;
  if (
    node._redSiCntResolved &&
    node._redSiCntResolvedFor === cntKey(st.folder || "", s + ext, digits)
  ) {
    rel = node._redSiCntResolved; // server-resolved (folder %counter% too)
  } else {
    const padded = String(node._redSiCounterNum || 1).padStart(digits, "0");
  ...
  // for a folder the user has since edited can never color the new one.
  if (
    node._redSiCntDenied &&
    node._redSiCntResolvedFor === cntKey(st.folder || "", s + ext, digits)
  ) {
    ui.prevPath.textContent = "This folder is not approved - click Browse and pick it once";
    ui.prevPath.style.color = "var(--red-acc)";
    ui.prevPath.title =
      "Reaper only writes to ComfyUI's own folders and to folders you picked " +
...
function restoreLastRun(node) {
  const ui = node._redSiUI;
  if (!ui) return;
  const last = node.properties?.redSiLastRun;
  if (!last || typeof last !== "object") return;
  if (last.w && last.h) node._redSiLastDims = { w: last.w, h: last.h };
  if (typeof last.sum === "string") node._redSiSummary = last.sum;
  if (typeof last.folder === "string") node._redSiFolderInfo = last.folder;
  const frames = entriesToFrames(last.entries);
  if (frames.length) {
    node._redSiFrames = frames;
    node._redSiSel = 0;
    node._redSiExpanded = false;
    node._redSiTotal = Math.max(last.n || 0, frames.length);
  }
  renderPreviewUI(node);
}
...
function wireEvents(node, ui) {
  ...
  for (const c of CHIPS) {
    const chip = el("button", "red-si-chip", c.label);
    chip.type = "button";
    chip.title = c.title;
  ...
  const clearChip = el("button", "red-si-chip", "✕ Clear");
  ...
  const reset = el("button", "red-si-chip", "↺ Reset");
  reset.type = "button";
  ...
  ui.root.addEventListener("contextmenu", (e) => {
    ...
    if (frames.length && ui.view.contains(t)) {
      const cell = t.closest ? t.closest(".red-si-cell") : null;
      if (cell && cell.dataset.i != null) {
        const i = parseInt(cell.dataset.i, 10);
        if (isFinite(i)) node._redSiSel = i;
      }
  ...
  ui.btnFolder.addEventListener("click", async () => {
    try {
      const r = await fetch(redApiUrl("/reaper/api/save_image/open_folder"), {
        method: "POST",
...
function setupNode(node) {
  ...
  try {
    ...
    if (!desc || desc.configurable !== false) {
      ...
      console.warn("[ReaperSaveImage] imgs not configurable; native preview may appear");
    }
  } catch {}
  ...
  installNodeAccent(node, ui.root);   // the face follows this node's accent color
  const widget = node.addDOMWidget("reaper_save_image", "custom", ui.root, {
    getValue: () => null,
    setValue: () => {},
    // Nodes 2.0 otherwise unmounts DOM widgets as soon as the canvas is
    // zoomed below 100%, leaving this node with an empty body.
    hideOnZoom: false,
  ...
  wireEvents(node, ui);
  try {
    node._redSiFloorOff = installResizeFloor(ui.root, () => measureFloor(ui));
  } catch {}
...
function installExecutedListener() {
  ...
  api.addEventListener("executed", ({ detail }) => {
    ...
    if (!node || node.comfyClass !== COMFY_CLASS || !node._redSiUI) return;
    const out = detail.output || {};
    const frames = out.reaper_save_frames || out.images;
    if (!Array.isArray(frames) || !frames.length) return;
    const status = frames[0]._reaper_status || null;
    ...
    node._redSiFrames = entriesToFrames(frames);
    node._redSiSel = 0;
    node._redSiExpanded = false; // batches land on the grid first
    node._redSiTotal = Math.max(status ? status.saved : 0, node._redSiFrames.length);

    if (status) {
      if (status.w && status.h) node._redSiLastDims = { w: status.w, h: status.h };
      ...
      node._redSiSummary = sum;
      node._redSiFolderInfo = status.folder || "";
      ...
      try {
        ...
        node.properties.redSiLastRun = {
          ok,
          sum,
          folder: node._redSiFolderInfo,
          entries: keep,
          n: node._redSiTotal,
          w: status.w,
          h: status.h,
    ...
    renderPreviewUI(node);
    node._redSiCntKey = null; // files landed on disk - refetch the counter
    updatePreview(node);
    growToFloor(node);
  });
}
...
function installGraphToPromptHook() {
  if (app._redSiGraphPatched) return;
  app._redSiGraphPatched = true;
...
app.registerExtension({
  ...
  getNodeMenuItems(node) {
    ...
    return [
      ...
      },
      {
        ...
        callback: () => {
          node._redSiSkipReassert = true; // an explicit user size choice
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    if (nodeType.prototype._redSiPatched) return; // hot-reload re-wrap guard
    nodeType.prototype._redSiPatched = true;
    ...
    nodeType.prototype.onConfigure = function (info) {
      ...
      if (saved && saved[1] > INFLATED_H) {
        console.warn(
          "[ReaperSaveImage] saved height " + Math.round(saved[1]) +
    ...
    nodeType.prototype.onConnectionsChange = function () {
      ...
      if (this._redSiUI) queueMicrotask(() => updatePreview(this));
      return r;
    };
    ...
    nodeType.prototype.onRemoved = function () {
      ...
      // built DOM subtree. Nulling _redSiUI below only makes apply() a no-op.
      try {
        this._redSiReassertOff?.();
      } catch {}
      closeSettingsPanelFor(this);
      if (_imgMenuNode === this) closeImageMenu();
      clearTimeout(this._redSiCntTimer);
      clearTimeout(this._redSiFlashT);
      this._redSiUI = null;
