// Load Image Info (Reaper) - the companion that unpacks Load Image (Reaper)'s image_info
...
const CLASS = "ReaperLoadImageInfo";
const MIN_W = 190;
const DEFAULT_W = 216;
...
app.registerExtension({
  name: "Reaper.LoadImageInfo",

  beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== CLASS) return;
    if (nodeType.prototype._redInfoPatched) return;
    nodeType.prototype._redInfoPatched = true;
    ...
    nodeType.prototype.onDrawForeground = function (ctx) {
      ...
      // isGraphLoading gate - see the same clamp in load_image (convention #7)
...
if (!app._reaperImageInfoExecPatched) {
  app._reaperImageInfoExecPatched = true;
  api.addEventListener("executed", ({ detail }) => {
    try {
      const entry = detail?.output?.reaper_image_info?.[0];
      if (!entry) return;
      ...
      node._redInfoData = { width: entry.width, height: entry.height, filename: entry.filename };
      node.setDirtyCanvas?.(true, true);
    } catch (e) {
      console.warn("[Load Image Info (Reaper)] readout failed:", (e && e.message) || e);
    }
  });
}
