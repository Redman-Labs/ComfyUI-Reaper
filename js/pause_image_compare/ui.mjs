...
export const CONTROLS_ONLY_MIN_H = HEADER_H;
...
function injectCSS() {
  ...
  s.textContent = `
    .red-lb-root { display:flex; flex-direction:column; flex:1 1 0; min-height:0;
      box-sizing:border-box; padding:6px; gap:6px; font:10px sans-serif; color:#ddd;
      overflow:hidden; }
    ...
    .red-lb-seg.pause-mode.active { background:#a40505; border-color:#a40505; color:#fff; }
    .red-lb-seg.pass-mode.active { background:#04a72a; border-color:#04a72a; color:#fff; }
    ...
    .red-lb-btn { flex:1 1 0; min-width:0; height:20px; line-height:18px; border-radius:4px;
      border:1px solid rgba(255,255,255,0.18); background:rgba(255,255,255,0.05);
      color:rgba(255,255,255,0.85); font:10px sans-serif; cursor:pointer; padding:0 6px;
    ...
    .red-lb-btn.primary:hover:not(:disabled) { background:#4591fa; border-color:#fff; }
    .red-lb-btn.continue-action:not(:disabled) { background:#04a72a; border-color:#04a72a; color:#fff; }
    .red-lb-btn.continue-action:hover:not(:disabled) { background:#08bd32; border-color:#75dc8d; color:#fff; }
    .red-lb-btn.regenerate-action:not(:disabled) { background:#e2ac06; border-color:#e2ac06; color:#111; }
    .red-lb-btn.regenerate-action:hover:not(:disabled) { background:#f2bd18; border-color:#ffe07a; color:#111; }
    ...
    .red-lb-empty { position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
      text-align:center; color:#777; font-size:10px; padding:8px; box-sizing:border-box; }
...
export function buildPauseWidget(node, callbacks) {
  injectCSS();
  const controlsOnly = node.comfyClass === "ReaperPauseImageCompare";
  ...
  segPause.className = "red-lb-seg pause-mode";
  ...
  segPass.className = "red-lb-seg pass-mode";
  ...
  btnContinue.className = "red-lb-btn primary continue-action";
  btnContinue.textContent = "▶ Continue";
  btnContinue.title = "Run only the rest of the workflow, from the snapshot";
  const btnRegen = document.createElement("button");
  btnRegen.className = "red-lb-btn regenerate-action";
  ...
  // The hybrid node uses Image Compare as its only visible image viewer.
  // Keep the Pause Image <img> mounted but hidden: showFrame() still loads the
  // Image 1 snapshot through it and enables Continue when that file exists.
  if (controlsOnly) {
    preview.style.display = "none";
    dims.style.display = "none";
    root.style.flex = `0 0 ${CONTROLS_ONLY_MIN_H}px`;
    root.style.height = `${CONTROLS_ONLY_MIN_H}px`;
    root.style.minHeight = `${CONTROLS_ONLY_MIN_H}px`;
    root.style.maxHeight = `${CONTROLS_ONLY_MIN_H}px`;
    // Let comparison hover/wipe events pass through every transparent part of
    // the controls layer. Re-enable pointer handling only on actual controls.
    root.style.pointerEvents = "none";
    toggle.style.pointerEvents = "auto";
    btns.style.pointerEvents = "auto";
    btns2.style.pointerEvents = "auto";
  }
