...
import {
  beginPauseSubmission,
  coordinatePauseMode,
  endPauseSubmission,
  markPauseReached,
} from "../shared/pause_submission.mjs";
import { getState, setGate, STATE_PROP } from "../shared/pause_image_state.mjs";
...
const CLASS = "ReaperPauseImageCompare";
const HIDDEN_INPUT = "PauseState";
const WIDGET_TYPE = "reaper_pause_image_compare_ui";
const HYBRID_MIN_H = CONTROLS_ONLY_MIN_H + 500;
const HYBRID_MIN_W = 500;

function isPauseClass(name) {
  return name === CLASS;
}

function gatedImageInput() {
  return "image1";
}

function enforceMinimumSize(node) {
  if (!node?.size) return;
  const minimumWidth = node.comfyClass === CLASS
    ? HYBRID_MIN_W
    : NODE_MIN_W;
  const minimumHeight = node.comfyClass === CLASS
    ? HYBRID_MIN_H
    : NODE_MIN_H;
  const width = Math.max(node.size[0], minimumWidth);
...
async function queueWithMode(node, mode) {
  ...
  for (const n of allNodes) {
    if (n === node) continue;
    if (n._redPauseSubmitMode) n._redPauseSubmitMode = null;
    if (n._redPtSubmitMode) n._redPtSubmitMode = null;
  }
  const submission = beginPauseSubmission(node, CLASS, mode);
  node._redPauseSubmitMode = mode;
  node._redPauseBusy = mode === "continue" ? "Continuing…" : "Regenerating…";
  renderPause(node);
  try {
    ...
    endPauseSubmission(submission);
    node._redPauseSubmitMode = null;
    node._redPauseBusy = null;
    renderPause(node);
  }
}
...
function flash(node, msg) {
  node._redPauseFlash = msg;
  renderPause(node);
  clearTimeout(node._redPauseFlashTimer);
  node._redPauseFlashTimer = setTimeout(() => {
    node._redPauseFlash = null;
    renderPause(node);
  }, 2000);
}

// Copy the previewed snapshot to the OS clipboard as PNG (one-click, like
// Preview Image Reaper - not ComfyUI's internal clipspace copy).
...
async function resolveSaveMeta(node) {
  const m = node._redPauseExecMeta;
...
async function saveToOutput(node) {
  if (!node._redPauseHasSnapshot) { flash(node, "Run once to capture an image"); return; }
  try {
    const image_b64 = await snapshotDataURL(node);
    const { workflow, prompt } = await resolveSaveMeta(node);
    const resp = await fetch("/reaper/api/preview/save", {
      method: "POST",
...
async function saveToDisk(node) {
  ...
  try {
    const image_b64 = await snapshotDataURL(node);
    const { workflow, prompt } = await resolveSaveMeta(node);
    const resp = await fetch("/reaper/api/preview/prepare", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ image_b64, filename_prefix: SAVE_PREFIX, workflow, prompt }),
    });
...
function setupNode(node) {
  ...
  const widget = node.addDOMWidget(WIDGET_TYPE, WIDGET_TYPE, root, {
    serialize: false,
    getMinHeight: () => (
      node.comfyClass === CLASS ? CONTROLS_ONLY_MIN_H : NODE_MIN_H
    ),
    getMaxHeight: () => (
      node.comfyClass === CLASS ? CONTROLS_ONLY_MIN_H : undefined
    ),
  });
  if (node.comfyClass === CLASS) {
    widget.computeSize = (width) => [
      width || HYBRID_MIN_W,
      CONTROLS_ONLY_MIN_H,
    ];
    // addDOMWidget supplies a default computeLayoutSize method. In Nodes 2.0
    // that makes this row flexible and it absorbs the node's spare height,
    // leaving a large gap before the comparison controls. Shadow the default
    // so only the comparison widget grows.
    widget.computeLayoutSize = undefined;

    // compare.mjs registers first (ES module imports are evaluated before this
    // file), so Nodes 2.0 initially appends the flexible comparison canvas
    // before these fixed-height controls. Reorder the node's widgets while it
    // is still being created: controls first, comparison canvas second. Vue
    // then lays every button out at the top and gives the remaining space to
    // the image viewer at the bottom.
    const widgets = node.widgets || [];
    const pauseIndex = widgets.indexOf(widget);
    const compareIndex = widgets.findIndex(
      (candidate) => candidate?.name === "reaper_compare",
    );
    if (
      pauseIndex !== -1
      && compareIndex !== -1
      && pauseIndex > compareIndex
    ) {
      widgets.splice(pauseIndex, 1);
      widgets.splice(compareIndex, 0, widget);
    }
  }
  ...
  if (!node.size || node.size[0] < NODE_MIN_W) node.size[0] = 440;
...
app.registerExtension({
  name: "Reaper.PauseImageCompare",

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (!isPauseClass(nodeData.name)) return;
    ...
    nodeType.prototype.onConfigure = function () {
      ...
      restore(this);
      queueMicrotask(() => enforceMinimumSize(this));
      return r;
    };
    ...
    nodeType.prototype.onResize = function (size) {
      const minimumWidth = this.comfyClass === CLASS
        ? HYBRID_MIN_W
        : NODE_MIN_W;
      if (size[0] < minimumWidth) size[0] = minimumWidth;
      const minimumHeight = this.comfyClass === CLASS
        ? HYBRID_MIN_H
        : NODE_MIN_H;
    ...
    nodeType.prototype.onRemoved = function () {
      clearTimeout(this._redPauseFlashTimer);
      this._redPauseRO?.disconnect();
      this._redPauseEls = null;
...
api.addEventListener("executed", (e) => {
  const d = e.detail;
  const frames = d?.output?.reaper_pause_frame;
  ...
  if (!node || !isPauseClass(node.comfyClass)) return;
  markPauseReached(node, CLASS);
  const f = frames[0];
  ...
  if (f._reaper_meta) node._redPauseExecMeta = f._reaper_meta;
  const s = getState(node);
...
function buildNodeIndex() {
  ...
  const visit = (graph) => {
    ...
    for (const n of nodes) {
      if (!n) continue;
      if (isPauseClass(n.comfyClass) || isPauseClass(n.type)) {
...
app.graphToPrompt = async function (...args) {
  ...
  if (out) {
    ...
    for (const id in out) {
      const entry = out[id];
      if (!entry || !isPauseClass(entry.class_type)) continue;
      ...
      const submit = node?._redPauseSubmitMode;
      let mode;
      ...
      mode = coordinatePauseMode(out, id, CLASS, mode);
      gates.push({
        id,
        entry,
        mode,
        imageInput: gatedImageInput(entry.class_type),
      });
    }
