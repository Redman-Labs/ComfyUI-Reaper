...
import { BRAND as DEFAULT_BRAND, registerNodeHelp,
...
// Canvas controls resolve the current Reaper Default Style at paint time.
...
const COMPARE_HELP = {
  ...
  footer: "Set the default view mode in Settings -> Reaper -> Image Compare.",
};
registerNodeHelp("ReaperPauseImageCompare", {
  ...COMPARE_HELP,
  title: "Pause Image Compare (Reaper)",
  tagline: "Pause Image controls plus interactive comparison against Image 2.",
});
...
const HYBRID_MIN_H = 630;

function imageTop(node) {
  return IMG_Y + (
    node.comfyClass === "ReaperPauseImageCompare" && !isVueNodes()
      ? HYBRID_CONTROLS_H
      : 0
  );
}
...
function cmpCursor(node, lx, ly, W, H) {
  ...
  if (ly >= imageTop(node) && node._cmpShowWhich === 0 && (node._cmpImg1 || node._cmpImg2)) {
...
// tooltip (same look as the DOM-button tooltips elsewhere in Reaper), driven
...
function paintBtn(ctx, r, label, on, hovered) {
  ...
  // no fill change (the Reaper node UI convention, CLAUDE.md #13). Active
...
// the border BRAND + brightens the text (Reaper node UI convention #13).
function paintUtilBtn(ctx, r, label, hover, flash) {
  ctx.save();
  // Hover = border BRAND + brighter text, NO fill (same as the mode buttons /
  // Reaper convention #13). Fill is reserved for state: green flash = success.
...
// text next to it carries the match/mismatch color.
...
const SETTING_DEFAULT_MODE = "Reaper.Compare.DefaultMode";
...
function cmpToast(msg) {
  ...
  else console.warn("[Reaper] Compare:", msg);
}
...
async function copyShownImage(node) {
  ...
  try {
    ...
    console.warn("[Reaper] Compare copy failed:", err);
...
async function saveShownToOutput(node) {
  ...
  try {
    ...
    console.warn("[Reaper] Compare save failed:", err);
...
async function saveShownToDisk(node) {
  ...
  try {
    ...
    console.warn("[Reaper] Compare prepare failed:", err);
...
function paintCompare(ctx, node, W, H, mouse) {
  BRAND = globalAccent() || DEFAULT_BRAND;
  const imgY = imageTop(node);
  ...
  const fit = (img) => {
    if (!img) return { x: 0, y: imgY, w: W, h: imgH };
    const a = img.naturalWidth / img.naturalHeight;
    const fh = W / a;
    if (fh <= imgH) return { x: 0, y: imgY + (imgH - fh) / 2, w: W, h: fh };
    const fw = imgH * a;
  ...
  if (m === 0 || m === 1) {
    ...
    if (node._cmpSplitX > 0.01 && node._cmpSplitX < 0.99) {
      ...
      ctx.beginPath();
      ctx.moveTo(sx, imgY);
      ctx.lineTo(sx, imgY + imgH);
      ctx.stroke();
    }
  } else if (m === 2) {
    const sy = imgY + imgH * node._cmpSplitY;
    if (node._cmpImg1) {
      ctx.save();
      ctx.beginPath();
      ctx.rect(0, sy, W, imgY + imgH - sy);
      ctx.clip();
...
function cmpMove(node, lx, ly, W, H) {
  ...
  if (
    ...
    const imgW = W,
      imgH = H - imageTop(node);
    if (node._cmpMode <= 1)
      node._cmpSplitX = Math.max(0, Math.min(1, lx / imgW));
    else node._cmpSplitY = Math.max(0, Math.min(1, (ly - imageTop(node)) / imgH));
    return true;
  }
  return false;
}
...
function createCompareDOMWidget(node) {
  ...
  const widget = node.addDOMWidget("reaper_compare", "reaper_compare", root, {
    serialize: false,
    hideOnZoom: false,
    getMinHeight: () => MIN_H,
  });
  ...
  widget.computeLayoutSize = () => (
    node.comfyClass === "ReaperPauseImageCompare"
      ? { minHeight: 500, minWidth: HYBRID_MIN_W }
      : { minHeight: MIN_H, minWidth: 1 }
  );
...
app.registerExtension({
  name: "Reaper.PauseImageCompare.Compare",
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "ReaperPauseImageCompare") return;
    ...
    nodeType.prototype.onDrawForeground = function (ctx) {
      ...
      // Enforce min size
      const minimumWidth = this.comfyClass === "ReaperPauseImageCompare"
        ? HYBRID_MIN_W
        : MIN_W;
      const minimumHeight = this.comfyClass === "ReaperPauseImageCompare"
        ? HYBRID_MIN_H
        : MIN_H;
      if (this.size[0] < minimumWidth) this.size[0] = minimumWidth;
      if (this.size[1] < minimumHeight) this.size[1] = minimumHeight;
    ...
    nodeType.prototype.onResize = function (e) {
      ...
      const minimumHeight = this.comfyClass === "ReaperPauseImageCompare"
        ...
        : 390;
      if (Array.isArray(e)) {
        e[0] = Math.max(e[0], minimumWidth);
        e[1] = Math.max(e[1], minimumHeight);
      }
      if (this.size) {
        this.size[0] = Math.max(this.size[0], minimumWidth);
        this.size[1] = Math.max(this.size[1], minimumHeight);
      }
