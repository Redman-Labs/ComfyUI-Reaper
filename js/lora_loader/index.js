// LoRA Loader Reaper - stack many LoRAs in one node. One DOM widget (Add / All /
  ...
  listLoras().then(() => {
    ...
    const walk = (g) => {
      for (const n of (g?._nodes || [])) {
        if ((n.comfyClass === CLASS || n.type === CLASS) && n._redLlRoot) renderNode(n);
...
function setupNode(node) {
  ...
  root.className = "red-ll-root";
  const inner = document.createElement("div");
  inner.className = "red-ll-inner";
  root.appendChild(inner);

  const widget = node.addDOMWidget("loras_ui", "reaper_lora_loader", root, {
  ...
  listLoras().then(() => { if (node._redLlRoot) renderNode(node); });
}

app.registerExtension({
  name: "Reaper.LoraLoader",

  beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== CLASS) return;
    if (nodeType.prototype._redLlPatched) return;
    nodeType.prototype._redLlPatched = true;
    ...
    nodeType.prototype.onConfigure = function (info) {
      ...
      if (this._redLlRoot) { renderNode(this); fitToContent(this); }
      return r;
    };
...
app.graphToPrompt = async function (...args) {
  ...
  try {
    ...
    console.warn("[LoRA Loader Reaper] could not inject state:", (e && e.message) || e);
  }
  return result;
};

registerNodeHelp(CLASS, {
  title: "LoRA Loader Reaper",
  ...
  sections: [
    ...
    },
    {
      ...
      body:
        ...
        "They are stored on this computer, alongside your other Reaper settings, so they survive an " +
    ...
    },
    {
      ...
      body:
        ...
        "is kept on this computer alongside your other Reaper settings, never written into your " +
    ...
    },
    {
      ...
      body:
        "XY Plot Reaper can drive this node. In its picker your rows show up under this " +
    ...
    },
    {
      ...
      body:
        ...
        "the Civitai button, thumbnails, your Civitai account, and the highlight color). Right-click a row " +
