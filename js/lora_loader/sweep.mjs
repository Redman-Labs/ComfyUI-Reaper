// LoRA Loader Reaper - the XY Plot sweep provider.
//
...
//
// Axis identity is `redlora:<row id>` + a subField ("name" | "sm" | "sc"). The row ID
...
function strengthOf(value) {
  ...
  if (!Number.isFinite(n)) {
    ...
    console.warn(`[LoRA Loader Reaper] XY Plot gave a strength that is not a number (${JSON.stringify(value)}); this square ran at ${r}.`);
    return r;
  }
  if (Math.abs(r - n) > 0.005) {
    console.warn(
      `[LoRA Loader Reaper] XY Plot asked for a strength of ${n}, but LoRA strengths ` +
...
function inject(entry, axis, value, node) {
  ...
  if (idx < 0) {
    ...
    console.warn("[LoRA Loader Reaper] XY Plot is set to vary a LoRA row that no longer exists on this node - re-pick the axis.");
    return;
  }
