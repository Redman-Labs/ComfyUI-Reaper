// LoRA Loader Reaper - all node-body events. ONE delegated set of listeners on
...
function injectMenuCSS() {
  if (document.getElementById("red-ll-menu-css")) return;
  const s = document.createElement("style");
  s.id = "red-ll-menu-css";
  s.textContent = `
    .red-ll-menu { position:fixed; z-index:10030; width:168px; background:#2b2b2b; border:1px solid #4a4a4a;
    ...
    .red-ll-menu .it { display:flex; align-items:center; gap:9px; padding:7px 12px; cursor:pointer; }
    .red-ll-menu .it .k { width:14px; text-align:center; color:#8a8a8a; }
    .red-ll-menu .it:hover { background:var(--acc,#f66744); color:#fff; } .red-ll-menu .it:hover .k { color:#fff; }
    .red-ll-menu .it.danger:hover { background:#e2504a; }
    .red-ll-menu .it.dis { opacity:.35; pointer-events:none; }
    .red-ll-menu .sep { height:1px; background:#1b1b1b; margin:3px 0; }
  `;
...
function rowIdOf(target) {
  const row = target.closest?.(".red-ll-row");
...
export function attachInteractions(node, widgetEl, refresh) {
  widgetEl.addEventListener("click", (ev) => {
    ...
    if (act === "add") {
      ...
      if (res.ok) {
        requestAnimationFrame(() => {
          const rowEl = widgetEl.querySelector(`.red-ll-row[data-id="${res.state.loras[res.index].id}"] .red-ll-name`);
    ...
    const id = rowIdOf(t);
    if (!id) return;
    if (act === "name") { openNamePicker(node, id, t.closest(".red-ll-name"), refresh); return; }
