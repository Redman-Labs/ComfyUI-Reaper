// LoRA Loader Reaper - the info panel (click a row's i). Shows the LoRA's info +
...
function injectCSS() {
  if (document.getElementById("red-ll-info-css")) return;
  const s = document.createElement("style");
  s.id = "red-ll-info-css";
  s.textContent = `
    .red-ll-info-p { position:fixed; z-index:10025; width:340px; max-width:94vw; background:#2b2b2b;
    ...
    .red-ll-info-top { display:flex; gap:11px; padding:12px; border-bottom:1px solid #1c1c1c; cursor:grab; }
    .red-ll-info-th { width:64px; height:64px; border-radius:7px; flex:none; border:1px solid #000;
    ...
    .red-ll-info-th::after { content:attr(data-hint); position:absolute; inset:0; display:flex;
    ...
    .red-ll-info-th:hover::after, .red-ll-info-th.drop::after, .red-ll-info-th.busy::after { opacity:1; }
    .red-ll-info-th.drop { border-color:var(--acc,${BRAND}); box-shadow:inset 0 0 0 2px var(--acc,${BRAND}); }
    .red-ll-thx { position:absolute; top:2px; right:2px; width:15px; height:15px; z-index:2;
    ...
    .red-ll-info-th:hover .red-ll-thx { opacity:1; }
    .red-ll-thx:hover { background:#e0604a; color:#fff; }
    .red-ll-info-h { min-width:0; flex:1; }
    .red-ll-info-h h3 { margin:0 0 4px; font-size:13.5px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .red-ll-info-meta { font:10px monospace; color:#7a7a7a; line-height:1.7; }
    .red-ll-civlink { display:inline-block; margin-top:5px; font:10.5px 'Segoe UI'; color:#8fc0ff;
      cursor:pointer; }
    .red-ll-civlink:hover { color:#b8d8ff; text-decoration:underline; }
    .red-ll-info-x { margin-left:auto; color:#8a8a8a; cursor:pointer; align-self:flex-start; }
    .red-ll-info-x:hover { color:#fff; }
    .red-ll-info-sec { padding:11px 12px; }
    .red-ll-info-sec h4 { margin:0 0 6px; font:600 9.5px 'Segoe UI'; text-transform:uppercase; letter-spacing:.7px;
      color:${BRAND}; display:flex; align-items:center; gap:7px; }
    .red-ll-info-sec h4 .src { margin-left:auto; font:9px 'Segoe UI'; text-transform:none; letter-spacing:0;
      color:#8a8a8a; border:1px solid #444; border-radius:99px; padding:1px 7px; }
    .red-ll-info-sec h4 .src.net { color:#8fc0ff; border-color:#3a5a80; }
    .red-ll-info-sec h4 .qa { margin-left:8px; font:9.5px 'Segoe UI'; text-transform:none; letter-spacing:0;
      color:#9a9a9a; cursor:pointer; }
    .red-ll-info-sec h4 .qa:hover { color:${BRAND}; }
    .red-ll-info-note { font-size:10px; color:#7a7a7a; margin:0 0 8px; }
    .red-ll-chips { display:flex; flex-wrap:wrap; gap:5px; max-height:36vh; overflow-y:auto; padding-right:2px; }
    .red-ll-chips::-webkit-scrollbar { width:7px; }
    .red-ll-chips::-webkit-scrollbar-thumb { background:#555; border-radius:3px; }
    .red-ll-chip { font:10.5px 'Segoe UI'; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.14);
    ...
    .red-ll-chip:hover { border-color:var(--acc,${BRAND}); }
    .red-ll-chip.sel { background:color-mix(in srgb, var(--acc,${BRAND}) 18%, transparent); border-color:var(--acc,${BRAND}); color:#f8a48c; }
    .red-ll-chip.sel::before { content:"✓"; font-size:9px; flex:none; }
    .red-ll-chip .ct { min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    .red-ll-chip-none { color:#777; font-size:11px; }
    .red-ll-chip .cx { margin-left:1px; color:#f8a48c; cursor:pointer; opacity:.6; font-size:10px; flex:none; }
    .red-ll-chip .cx:hover { opacity:1; }
    .red-ll-srctoggle { margin-left:auto; display:flex; border:1px solid #444; border-radius:99px; overflow:hidden; }
    .red-ll-srctoggle .sg { font:9px 'Segoe UI'; text-transform:none; letter-spacing:0; color:#9a9a9a; padding:2px 9px; cursor:pointer; }
    .red-ll-srctoggle .sg:hover { color:#ddd; }
    .red-ll-srctoggle .sg.on { background:${BRAND}; color:#fff; }
    .red-ll-addtrig { display:flex; gap:5px; margin-top:8px; }
    .red-ll-addtrig input { flex:1; min-width:0; box-sizing:border-box; background:#161616;
      border:1px solid rgba(255,255,255,0.14); border-radius:6px; color:#fff; font:11px 'Segoe UI';
      padding:5px 8px; outline:none; }
    .red-ll-addtrig input:focus { border-color:${BRAND}; }
    .red-ll-addtrig button { flex:0 0 auto; background:rgba(255,255,255,0.06);
      border:1px solid rgba(255,255,255,0.14); color:#ccc; border-radius:6px; padding:5px 11px;
      font:11px 'Segoe UI'; cursor:pointer; }
    .red-ll-addtrig button:hover { border-color:${BRAND}; color:#fff; }
    .red-ll-strip { margin:0 12px 11px; border-radius:7px; padding:9px 10px; font-size:11px; line-height:1.5;
      display:flex; gap:9px; align-items:flex-start; }
    .red-ll-strip .st-ic { flex:none; width:18px; height:18px; border-radius:50%; color:#fff; font-size:11px;
      display:flex; align-items:center; justify-content:center; }
    .red-ll-strip.searching { background:rgba(90,160,230,0.12); } .red-ll-strip.searching .st-ic { background:#5aa0e6; }
    .red-ll-strip.found { background:rgba(62,195,113,0.12); } .red-ll-strip.found .st-ic { background:#3ec371; }
    .red-ll-strip.nofind { background:rgba(255,255,255,0.05); } .red-ll-strip.nofind .st-ic { background:#6f6f6f; }
    .red-ll-strip.offline { background:rgba(233,165,61,0.12); } .red-ll-strip.offline .st-ic { background:#e9a53d; }
    .red-ll-spin { width:11px; height:11px; border:2px solid rgba(255,255,255,.3); border-top-color:#fff;
      border-radius:50%; animation:red-ll-sp 1s linear infinite; }
    @keyframes red-ll-sp { to { transform:rotate(360deg); } }
    .red-ll-info-foot { display:flex; gap:6px; padding:10px 12px; border-top:1px solid #1c1c1c; background:#242424; }
    .red-ll-info-foot .b { flex:1; text-align:center; font-size:11px; padding:7px; border-radius:5px; cursor:pointer; }
    .red-ll-info-foot .b.pri { background:${BRAND}; color:#fff; font-weight:600; }
    .red-ll-info-foot .b.gh { border:1px solid rgba(255,255,255,0.14); color:#b8b8b8; }
    .red-ll-info-foot .b.gh:hover { border-color:${BRAND}; color:#fff; }
    .red-ll-info-foot .b.dis { opacity:.4; pointer-events:none; }
    .red-ll-info-foot .b.del { flex:0 0 auto; min-width:38px; border:1px solid rgba(255,255,255,0.14); color:#c9736a; }
    .red-ll-info-foot .b.del:hover { border-color:#e0604a; color:#fff; background:rgba(224,96,74,0.12); }
  `;
...
// `.red-ll-info-p` is position:fixed with no left/top, so until this runs it sits
...
export async function openInfoPanel(node, id, refresh) {
  ...
  const panel = el("div", "red-ll-info-p");
  ...
  function addCustom(word) {
    ...
    refresh?.(false);
    renderBody();
    setTimeout(() => panel.querySelector(".red-ll-addtrig input")?.focus(), 0);
  }
  ...
  function setBusy(v) {
    _busy = !!v;
    const t = panel.querySelector(".red-ll-info-th");
    if (!t) return;
  ...
  function wirePanelDrop(p) {
    ...
    p.addEventListener("drop", (ev) => {
      ...
      if (!ev.target.closest?.(".red-ll-addtrig")) {
  ...
  function wireThumb(th, hasOwn) {
    ...
    th.addEventListener("click", (ev) => {
      if (ev.target.closest(".red-ll-thx")) return;   // the remove badge has its own job
      pickPicture();
    });
    if (hasOwn) {
      const rm = el("span", "red-ll-thx", "✕");
  ...
  function msgStrip() {
    const strip = el("div", "red-ll-strip offline");
  ...
  function renderBody() {
    ...
    // ── header ───────────────────────────────────────────────────────────
    const top = el("div", "red-ll-info-top");
    ...
    const meta = el("div", "red-ll-info-meta");
    ...
    if (mid != null) {
      const link = el("span", "red-ll-civlink", "View on Civitai ↗");
    ...
    // ── trigger words ────────────────────────────────────────────────────
    const sec = el("div", "red-ll-info-sec");
    const head = el("h4");
    ...
    if (civitaiAvailable() && (info.file_triggers?.length || 0) > 0) {
      ...
      const seg = el("div", "red-ll-srctoggle");
    ...
    const addRow = el("div", "red-ll-addtrig");
    const inp = el("input");
    inp.type = "text";
    ...
    // ── footer ───────────────────────────────────────────────────────────
    const foot = el("div", "red-ll-info-foot");
  ...
  const onDown = (e) => { if (!panel.contains(e.target) && !e.target.closest?.(".red-ll-dd")) closeInfoPanel(); };
...
function dragBy(panel) {
  ...
  panel.addEventListener("pointerdown", (e) => {
    if (!e.target.closest?.(".red-ll-info-top")) return;
    ...
    if (e.target.closest(".red-ll-info-x, .red-ll-civlink, .red-ll-info-th")) return;
