// LoRA Loader Reaper - thin fetch wrappers over the server routes in
...
export async function listLoras(force = false) {
  ...
  const p = (async () => {
    try {
      ...
      const r = await fetch(redApiUrl("/reaper/api/lora/list"), { cache: "no-store" });
...
export async function loraInfo(name, force = false) {
  ...
  p = (async () => {
    try {
      const r = await fetch(redApiUrl("/reaper/api/lora/info?name=" + encodeURIComponent(name)),
                            { cache: "no-store" });
      const j = await r.json();
...
export function thumbUrl(name, bust) {
  return redApiUrl("/reaper/api/lora/thumb?name=" + encodeURIComponent(name) +
    (bust ? "&t=" + bust : ""));
}

export async function civitaiLookup(name) {
  try {
    const r = await fetch(redApiUrl("/reaper/api/lora/civitai?name=" + encodeURIComponent(name)));
    return await r.json();
  } catch {
...
export async function getCivitaiAccount() {
  try {
    const r = await fetch(redApiUrl("/reaper/api/civitai/account"), { cache: "no-store" });
    return await r.json();
  } catch {
...
export async function setCivitaiAccount(patch) {
  try {
    const r = await fetch(redApiUrl("/reaper/api/civitai/account"), {
      method: "POST",
...
export async function saveCustomTriggers(name, words) {
  try {
    const r = await fetch(redApiUrl("/reaper/api/lora/custom_triggers"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function deleteCivitai(name) {
  try {
    const r = await fetch(redApiUrl("/reaper/api/lora/civitai_delete"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function saveLoraPreview(name, dataUrl) {
  try {
    const r = await fetch(redApiUrl("/reaper/api/lora/preview"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function deleteLoraPreview(name) {
  try {
    const r = await fetch(redApiUrl("/reaper/api/lora/preview_delete"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
    });
