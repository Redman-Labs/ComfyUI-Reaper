// LoRA Loader Reaper - the floating gear settings panel (Sizes / Sliders pattern:
...
function injectCSS() {
  if (document.getElementById("red-llp-css")) return;
  const s = document.createElement("style");
  s.id = "red-llp-css";
  s.textContent = `
    .red-llp { position:fixed; z-index:10010; width:290px; max-width:94vw; background:#1a1a1a;
    ...
    .red-llp-t { display:flex; align-items:center; gap:8px; padding:10px 12px; background:#232323;
      border-bottom:1px solid #333; cursor:grab; user-select:none; color:#fff; }
    .red-llp-t .x { margin-left:auto; color:#8a8a8a; cursor:pointer; padding:0 4px; }
    .red-llp-t .x:hover { color:#fff; }
    .red-llp-b { padding:12px; display:flex; flex-direction:column; gap:11px; max-height:64vh; overflow-y:auto; }
    .red-llp-row { display:flex; align-items:center; gap:10px; }
    .red-llp-row .lab { flex:1; color:#c2c2c2; }
    .red-llp-row .hint { display:block; font-size:10px; color:#7a7a7a; margin-top:1px; }
    .red-llp-num { width:66px; box-sizing:border-box; background:#161616; border:1px solid #4a4a4a;
      border-radius:6px; color:#fff; text-align:center; font:12px monospace; padding:6px 4px; outline:none; }
    .red-llp-num:focus { border-color:var(--acc,${BRAND}); }
    .red-llp-txt { width:70px; box-sizing:border-box; background:#161616; border:1px solid #4a4a4a;
      border-radius:6px; color:#fff; text-align:center; font:12px monospace; padding:6px 4px; outline:none; }
    .red-llp-txt:focus { border-color:var(--acc,${BRAND}); }
    .red-llp-sw { flex:0 0 auto; width:34px; height:18px; border-radius:99px; background:#3a3a3a;
      position:relative; cursor:pointer; border:1px solid #000; }
    .red-llp-sw::after { content:""; position:absolute; top:1px; left:1px; width:14px; height:14px;
      border-radius:50%; background:#8a8a8a; transition:left .14s, background .14s; }
    .red-llp-sw.on { background:var(--acc,${BRAND}); } .red-llp-sw.on::after { left:17px; background:#fff; }
    .red-llp-swatch { width:30px; height:22px; border-radius:5px; border:1px solid #555; cursor:pointer; flex:0 0 auto; }
    .red-llp-swatch:hover { border-color:#fff; }
    .red-llp-seg { flex:0 0 auto; display:flex; background:rgba(0,0,0,0.25); border:1px solid #444;
      border-radius:6px; overflow:hidden; }
    .red-llp-segb { padding:5px 9px; font:11px 'Segoe UI',sans-serif; color:#aaa; cursor:pointer;
      user-select:none; }
    .red-llp-segb:hover { color:#ddd; background:rgba(255,255,255,0.08); }
    .red-llp-segb.on { background:var(--acc,${BRAND}); color:#fff; }
    .red-llp-f { display:flex; gap:8px; padding:10px 12px; border-top:1px solid #333; background:#1f1f1f; }
    .red-llp-btn { border:1px solid #444; background:rgba(255,255,255,0.04); color:#d8d8d8; border-radius:5px;
      padding:6px 12px; font:12px 'Segoe UI',sans-serif; cursor:pointer; }
    .red-llp-btn:hover { border-color:var(--acc,${BRAND}); color:#fff; }
    .red-llp-push { margin-left:auto; }
    ...
    .red-llp-head { margin-top:2px; padding-top:11px; border-top:1px solid #333;
      color:var(--acc,${BRAND}); font-size:11px; letter-spacing:.04em; text-transform:uppercase; }
    .red-llp-head .sub { display:block; margin-top:3px; text-transform:none; letter-spacing:0;
      color:#7a7a7a; font-size:10px; line-height:1.4; }
    .red-llp-key { flex:1; min-width:0; box-sizing:border-box; background:#161616;
      border:1px solid #4a4a4a; border-radius:6px; color:#fff; font:12px monospace;
      padding:6px 8px; outline:none; }
    .red-llp-key:focus { border-color:var(--acc,${BRAND}); }
    .red-llp-mini { flex:0 0 auto; border:1px solid #444; background:rgba(255,255,255,0.04);
      color:#d8d8d8; border-radius:5px; padding:5px 9px; font:11px 'Segoe UI',sans-serif;
      cursor:pointer; user-select:none; }
    .red-llp-mini:hover { border-color:var(--acc,${BRAND}); color:#fff; }
    .red-llp-state { flex:1; font-size:11px; color:#7a7a7a; }
    .red-llp-state.set { color:#3ec371; }
    .red-llp-msg { font-size:10px; line-height:1.4; color:#c98a6a; }
  `;
...
function startFollowing(panel, node) {
  ...
  const tick = () => {
    ...
    // Park while the color picker is open, for the same reason as a user drag: the
    ...
    if (document.querySelector(".rl-cp-popup, .rl-cp-modal-backdrop")) return;
...
export function openLoraPanel(node, refresh) {
  ...
  function numRow(label, key, { min = 0, round = null } = {}) {
    ...
    const inp = el("input", "red-llp-num");
    inp.type = "text";
  ...
  // separator (text)
  const sepRow = el("div", "red-llp-row");
  sepRow.appendChild(el("div", "lab", "Trigger words separator"));
  const sepIn = el("input", "red-llp-txt");
  sepIn.type = "text";
  ...
  function segRow(label, key, options) {
    ...
    for (const o of options) {
      const b = el("div", "red-llp-segb", o.label);
      b.dataset.v = o.v;
      b.title = o.title;
  ...
  {
    const head = el("div", "red-llp-head", "Civitai account");
    ...
    const msg = el("div", "red-llp-msg");
    msg.style.display = "none";
    // `ok` picks the color. Sitting directly under the key row, a success note in
    ...
    // had worked. Green is the suite's success color (convention #2).
    const say = (t, ok) => {
      msg.textContent = t || "";
    ...
    let editing = false;
    const keyRow = el("div", "red-llp-row");
    const paintKeyRow = () => {
      editing = false;
      keyRow.textContent = "";
      const state = el("div", "red-llp-state" + (acc.configured ? " set" : ""),
        acc.configured ? "✓ Key saved  " + acc.hint : "No key - anonymous lookups");
      const edit = el("div", "red-llp-mini", acc.configured ? "Change" : "Add key");
      ...
      if (acc.configured) {
        const rm = el("div", "red-llp-mini", "Remove");
    ...
    function showEditor() {
      ...
      say("");
      keyRow.textContent = "";
      const inp = el("input", "red-llp-key");
      ...
      const ok = el("div", "red-llp-mini", "Save");
      const no = el("div", "red-llp-mini", "Cancel");
      const commit = () => {
    ...
    const hostRow = el("div", "red-llp-row");
    ...
    const hostSeg = el("div", "red-llp-seg");
    const HOSTS = [
    ...
    const adultRow = el("div", "red-llp-row");
    ...
    const adultSw = el("div", "red-llp-sw");
  ...
  // accent
  const accRow = el("div", "red-llp-row");
  accRow.appendChild(el("div", "lab", "Highlight color"));
  const sw = el("div", "red-llp-swatch");
  sw.style.background = accentOf(node);
  sw.title = "Pick the highlight color";
  sw.addEventListener("click", () => {
    ...
    _cpHandle = openReaperColorPickerPopup(sw, {
      ...
      onPick: (c) => {
        ...
        node._redLlInner?.style.setProperty("--acc", col);
        fire();
      },
    });
  });
  ...
  // footer
  const foot = el("div", "red-llp-f");
  const mkDefault = el("button", "red-llp-btn", "Set as default");
  ...
  const done = el("button", "red-llp-btn red-llp-push", "Done");
  ...
  const mkAll = el("button", "red-llp-btn", "Every Reaper node");
